...
kp_expected_runtime_lock="sha256:$(printf 'kuberploy-runtime-lock-v1|0.1.0-rc.413|%s' "${kp_expected_runtime_digest}" | shasum -a 256 | cut -d ' ' -f 1)"
...
text = text.replace(
    "annotations:\n",
    "annotations:\n"
    '  kuberploy.io/runtime-chart-version: "0.1.0-rc.413"\n'
...
[[ -f "${kp_chart}/charts/kuberploy-argocd-0.1.0-rc.413.tgz" ]]
[[ -f "${kp_chart}/charts/kuberploy-valkey-0.1.0-rc.413.tgz" ]]
python3 - "${kp_chart}" <<'PY'
import hashlib
import re
import sys
...
helm template kuberploy-installer "${kp_chart}" --namespace kuberploy-system -f "${kp_managed}" --is-upgrade \
	--set-string source.valuesRevision=v0.1.0-rc.413-upgrade \
	--set-string components.postgresql.expectedPackageVersion=0.1.0-rc.413 \
...
[[ "$(yq eval-all 'select(.kind == "Application") | .spec.sources[0].targetRevision' "${kp_tmp}/managed.yaml")" == "0.1.0-rc.413" ]]
...
[[ "$(yq eval-all 'select(.kind == "Application") | .spec.sources[1].targetRevision' "${kp_tmp}/managed.yaml")" == "v0.1.0-rc.413" ]]
...
[[ "$(yq eval-all 'select(.kind == "Job" and .metadata.name == "kuberploy-installer-application-reconciler") | .metadata.annotations."helm.sh/hook-delete-policy"' "${kp_tmp}/managed.yaml")" == "before-hook-creation,hook-succeeded" ]]
...
helm template kuberploy-installer "${kp_chart}" --namespace kuberploy-system -f "${kp_managed}" \
  --set bootstrap.applicationHealthTimeoutSeconds=3540 >"${kp_tmp}/managed-max-health-timeout.yaml"
[[ "$(yq eval-all 'select(.kind == "Job" and .metadata.name == "kuberploy-installer-application-health") | .spec.activeDeadlineSeconds' "${kp_tmp}/managed-max-health-timeout.yaml")" == "3600" ]]
[[ "$(yq eval-all 'select(.kind == "Job" and .metadata.name == "kuberploy-installer-application-health") | [.spec.template.spec.containers[].args[2]] | unique | join(",")' "${kp_tmp}/managed-max-health-timeout.yaml")" == "--timeout=3540s" ]]
...
[[ "$(yq eval-all 'select(.kind == "Job" and .metadata.name == "kuberploy-installer-application-health") | [.spec.template.spec.containers[].args[1]] | join(",")' "${kp_tmp}/managed.yaml")" == '--for=jsonpath={.spec.sources[0].targetRevision}=0.1.0-rc.413,--for=jsonpath={.status.sync.revisions[0]}=0.1.0-rc.413,--for=jsonpath={.status.sync.status}=Synced,--for=jsonpath={.status.health.status}=Healthy' ]]
...
[[ "$(yq eval-all -r 'select(.kind == "ConfigMap" and .metadata.labels."app.kubernetes.io/component" == "application-inventory") | .data."inventory.json"' "${kp_tmp}/managed-upgrade.yaml" | jq -r '.applications[0].targetPackageVersion')" == "0.1.0-rc.413" ]]
...
[[ "$(yq eval-all 'select(.kind == "Job" and .metadata.name == "kuberploy-installer-application-health") | [.spec.template.spec.containers[].args[1]] | join(",")' "${kp_tmp}/managed-upgrade.yaml")" == '--for=jsonpath={.spec.sources[0].targetRevision}=0.1.0-rc.413,--for=jsonpath={.status.sync.revisions[0]}=0.1.0-rc.413,--for=jsonpath={.status.sync.status}=Synced,--for=jsonpath={.status.health.status}=Healthy' ]]
...
[[ "$(yq eval-all 'select(.kind == "Job" and .metadata.name == "kuberploy-installer-application-health") | [.spec.template.spec.containers[].args[1]] | join(",")' "${kp_tmp}/managed-rollback.yaml")" == '--for=jsonpath={.spec.sources[0].targetRevision}=0.1.0-rc.413,--for=jsonpath={.status.sync.revisions[0]}=0.1.0-rc.413,--for=jsonpath={.status.sync.status}=Synced,--for=jsonpath={.status.health.status}=Healthy' ]]
[[ "$(yq eval-all '[select(.kind == "Secret")] | length' "${kp_tmp}/managed.yaml" | tail -1)" == "7" ]]
...
helm template kuberploy-installer "${kp_chart}" --namespace kuberploy-system -f "${kp_managed}" \
  ...
  --set-string components.controlPlane.expectedPackageVersion=0.1.0-rc.413 \
  --set components.valkey.enabled=true \
  --set components.valkey.mode=managed \
  --set-string components.valkey.expectedPackageVersion=0.1.0-rc.413 \
...
for kp_component in controlPlane postgresql valkey edge certManager externalDNS externalSecrets sealedSecrets monitoring builder registry; do
  ...
  kp_all_args+=(--set-string "components.${kp_component}.expectedPackageVersion=0.1.0-rc.413")
done
...
[[ "$(yq eval-all 'select(.kind == "Job" and .metadata.name == "kuberploy-installer-application-health") | [.spec.template.spec.containers[].args[1] | select(. == "--for=jsonpath={.spec.sources[0].targetRevision}=0.1.0-rc.413")] | length' "${kp_tmp}/all-components.yaml")" == "10" ]]
[[ "$(yq eval-all 'select(.kind == "Job" and .metadata.name == "kuberploy-installer-application-health") | [.spec.template.spec.containers[].args[1] | select(. == "--for=jsonpath={.status.sync.revisions[0]}=0.1.0-rc.413")] | length' "${kp_tmp}/all-components.yaml")" == "10" ]]
...
[[ "$(yq eval-all 'select(.kind == "Application" and .metadata.name == "kuberploy-registry") | .spec.sources[0].helm.valuesObject.exposure.manageCertificate' "${kp_tmp}/all-components.yaml")" == "true" ]]
...
[[ "$(yq eval-all 'select(.kind == "AppProject" and .metadata.name == "kuberploy-control-plane") | [.spec.destinations[] | select(.server == "https://kubernetes.default.svc") | .namespace] | contains(["kuberploy-helm-renderer"])' "${kp_tmp}/all-components.yaml")" == "false" ]]
...
helm template kuberploy-installer "${kp_chart}" --namespace kuberploy-system -f "${kp_managed}" "${kp_all_args[@]}" \
  --set publicEndpoint.tls.manageCertificate=false \
  --set integrations.registry.manageCertificate=false \
  >"${kp_tmp}/existing-certificates.yaml"
kp_expand_installer_applications "${kp_tmp}/existing-certificates.yaml"
[[ "$(yq eval-all 'select(.kind == "Application" and .metadata.name == "kuberploy-control-plane") | .spec.sources[0].helm.valuesObject.ingress.tls.secretName' "${kp_tmp}/existing-certificates.yaml")" == "kuberploy-platform-tls" ]]
[[ "$(yq eval-all 'select(.kind == "Application" and .metadata.name == "kuberploy-control-plane") | .spec.sources[0].helm.valuesObject.ingress.tls.issuerName' "${kp_tmp}/existing-certificates.yaml")" == "" ]]
[[ "$(yq eval-all 'select(.kind == "Application" and .metadata.name == "kuberploy-registry") | .spec.sources[0].helm.valuesObject.exposure.manageCertificate' "${kp_tmp}/existing-certificates.yaml")" == "false" ]]
[[ "$(yq eval-all 'select(.kind == "Application" and .metadata.name == "kuberploy-cert-manager") | (.spec.sources[0].helm.valuesObject.foundation.issuers.production.enabled // false)' "${kp_tmp}/existing-certificates.yaml")" == "false" ]]
yq eval-all 'select(.kind == "Application" and .metadata.name == "kuberploy-control-plane") | .spec.sources[0].helm.valuesObject' \
  "${kp_tmp}/existing-certificates.yaml" >"${kp_tmp}/existing-platform-certificate-values.yaml"
helm template kuberploy "${kp_root}/charts/kuberploy" --namespace kuberploy-system \
  -f "${kp_tmp}/existing-platform-certificate-values.yaml" >"${kp_tmp}/existing-platform-certificate.yaml"
[[ "$(yq eval-all '[select(.kind == "Ingress" and .metadata.name == "kuberploy") | .metadata.annotations."cert-manager.io/cluster-issuer"] | length' "${kp_tmp}/existing-platform-certificate.yaml" | tail -1)" == "0" ]]
[[ "$(yq eval-all 'select(.kind == "Ingress" and .metadata.name == "kuberploy") | .spec.tls[0].secretName' "${kp_tmp}/existing-platform-certificate.yaml")" == "kuberploy-platform-tls" ]]
yq eval-all 'select(.kind == "Application" and .metadata.name == "kuberploy-registry") | .spec.sources[0].helm.valuesObject' \
  "${kp_tmp}/existing-certificates.yaml" >"${kp_tmp}/existing-registry-certificate-values.yaml"
helm template registry "${kp_root}/charts/kuberploy-registry" --namespace kuberploy-system \
  -f "${kp_tmp}/existing-registry-certificate-values.yaml" >"${kp_tmp}/existing-registry-certificate.yaml"
[[ "$(yq eval-all '[select(.kind == "Certificate")] | length' "${kp_tmp}/existing-registry-certificate.yaml" | tail -1)" == "0" ]]
[[ "$(yq eval-all 'select(.kind == "Ingress") | .spec.tls[0].secretName' "${kp_tmp}/existing-registry-certificate.yaml")" == "registry-tls" ]]
...
kp_platform_args=(
  ...
  --set-string components.edge.expectedPackageVersion=0.1.0-rc.413
  --set components.certManager.enabled=true
  --set components.certManager.mode=managed
  --set-string components.certManager.expectedPackageVersion=0.1.0-rc.413
  --set components.sealedSecrets.enabled=true
  --set components.sealedSecrets.mode=managed
  --set-string components.sealedSecrets.expectedPackageVersion=0.1.0-rc.413
  --set components.monitoring.enabled=true
  --set components.monitoring.mode=managed
  --set-string components.monitoring.expectedPackageVersion=0.1.0-rc.413
...
[[ "$(yq eval-all 'select(.kind == "Ingress" and .metadata.name == "kuberploy") | [.metadata.annotations."cert-manager.io/private-key-algorithm",.metadata.annotations."cert-manager.io/private-key-size"] | join(",")' "${kp_tmp}/platform-tls-control.yaml")" == "ECDSA,256" ]]
...
kp_github_args=(
  ...
  --set-string components.builder.expectedPackageVersion=0.1.0-rc.413
...
[[ "$(yq eval-all 'select(.kind == "Application" and .metadata.name == "kuberploy-control-plane") | .spec.sources[0].helm.valuesObject.config.gitProjection.chartVersion' "${kp_tmp}/github-platform.yaml")" == "0.1.0-rc.413" ]]
[[ "$(yq eval-all 'select(.kind == "Application" and .metadata.name == "kuberploy-control-plane") | .spec.sources[0].helm.valuesObject.config.argoDesiredState.runtimeChartVersion' "${kp_tmp}/github-platform.yaml")" == "0.1.0-rc.413" ]]
...
[[ "$(yq eval-all 'select(.kind == "AppProject" and .metadata.name == "kuberploy-control-plane") | [.spec.destinations[].namespace] | contains(["kp-example-development"])' "${kp_tmp}/github-platform.yaml")" == "false" ]]

kp_runtime_secret_args=(
  --set integrations.runtimeSecrets.enabled=true
  --set-string integrations.runtimeSecrets.namespaces[0]=kp-example-development
  --set-string integrations.runtimeSecrets.namespaces[1]=kp-example-production
  --set-string integrations.runtimeSecrets.fingerprintKeyID=runtime-secrets-hmac-v1
  --set-string integrations.runtimeSecrets.fingerprintSecretName=kuberploy-runtime-secrets-fingerprint
  --set-string integrations.runtimeSecrets.fingerprintSecretKey=runtime-secret-fingerprint.key
  --set-string integrations.runtimeSecrets.sealingCertificateSecretName=kuberploy-runtime-secrets-sealing-certificate
  --set-string integrations.runtimeSecrets.sealingCertificateSecretKey=tls.crt
)
helm template kuberploy-installer "${kp_chart}" --namespace kuberploy-system -f "${kp_managed}" "${kp_platform_args[@]}" \
  --set publicEndpoint.tls.enabled=true \
  --set-string publicEndpoint.tls.secretName=kuberploy-platform-tls \
  --set-string publicEndpoint.tls.clusterIssuerName=kuberploy-letsencrypt-production \
  --set-string publicEndpoint.tls.accountEmail=platform@example.com \
  "${kp_github_args[@]}" "${kp_runtime_secret_args[@]}" >"${kp_tmp}/runtime-secrets-platform.yaml"
kp_expand_installer_applications "${kp_tmp}/runtime-secrets-platform.yaml"
[[ "$(yq eval-all 'select(.kind == "AppProject" and .metadata.name == "kuberploy-control-plane") | [.spec.destinations[].namespace] | sort | join(",")' "${kp_tmp}/runtime-secrets-platform.yaml")" == "argocd,cert-manager,kp-*,kp-example-development,kp-example-production,kuberploy-build-dind,kuberploy-monitoring,kuberploy-system" ]]
[[ "$(yq eval-all 'select(.kind == "Application" and .metadata.name == "kuberploy-control-plane") | .spec.sources[0].helm.valuesObject.config.runtimeSecrets.enabled' "${kp_tmp}/runtime-secrets-platform.yaml")" == "true" ]]
[[ "$(yq eval-all 'select(.kind == "Application" and .metadata.name == "kuberploy-control-plane") | .spec.sources[0].helm.valuesObject.config.runtimeSecrets.namespaces | sort | join(",")' "${kp_tmp}/runtime-secrets-platform.yaml")" == "kp-example-development,kp-example-production" ]]
[[ "$(yq eval-all 'select(.kind == "Application" and .metadata.name == "kuberploy-control-plane") | .spec.sources[0].helm.valuesObject.config.runtimeSecrets.fingerprintSecret.name' "${kp_tmp}/runtime-secrets-platform.yaml")" == "kuberploy-runtime-secrets-fingerprint" ]]
[[ "$(yq eval-all 'select(.kind == "Application" and .metadata.name == "kuberploy-control-plane") | .spec.sources[0].helm.valuesObject.config.runtimeSecrets.sealingCertificateSecret.name' "${kp_tmp}/runtime-secrets-platform.yaml")" == "kuberploy-runtime-secrets-sealing-certificate" ]]
[[ "$(yq eval-all 'select(.kind == "Application" and .metadata.name == "kuberploy-control-plane") | .spec.sources[0].helm.valuesObject.config.certificateObservation.enabled' "${kp_tmp}/runtime-secrets-platform.yaml")" == "true" ]]
if helm template invalid-runtime-secrets "${kp_chart}" --namespace kuberploy-system -f "${kp_managed}" "${kp_platform_args[@]}" \
  "${kp_github_args[@]}" "${kp_runtime_secret_args[@]}" \
  --set-string integrations.runtimeSecrets.namespaces[0]=kuberploy-system >/dev/null 2>&1; then
  printf 'installer accepted a reserved platform namespace for runtime secrets\n' >&2
  exit 1
fi
...
kp_registry_pull_args=(
  ...
  --set-string components.registry.expectedPackageVersion=0.1.0-rc.413
...
[[ "$(yq eval-all 'select(.kind == "Application" and .metadata.name == "kuberploy-builder") | .spec.sources[0].targetRevision' "${kp_tmp}/github-platform.yaml")" == "0.1.0-rc.413" ]]
...
helm template kuberploy "${kp_root}/charts/kuberploy" --namespace kuberploy-system \
  ...
  --set-string builder.builderAgentImage=ghcr.io/kuberploy/kuberploy-builder-agent:0.1.0-rc.413 \
...
helm template kuberploy-installer "${kp_chart}" --namespace kuberploy-system -f "${kp_adopted}" \
  --show-only templates/bootstrap-secrets.yaml --set components.controlPlane.enabled=true \
  --set components.controlPlane.mode=managed --set-string components.controlPlane.expectedPackageVersion=0.1.0-rc.413 \
  --set components.postgresql.enabled=true --set components.postgresql.mode=adopted --set components.postgresql.adoptionConfirmed=true --set-string components.postgresql.expectedPackageVersion=0.1.0-rc.413 \
  --set components.valkey.enabled=true --set components.valkey.mode=adopted --set components.valkey.adoptionConfirmed=true --set-string components.valkey.expectedPackageVersion=0.1.0-rc.413 \
  --set bootstrap.controlPlaneToken.mode=generated --set-string bootstrap.controlPlaneToken.kubeAPIServerCIDRs[0]=10.43.0.1/32 \
  >"${kp_tmp}/adopted-control-plane-secrets.yaml"
[[ "$(yq eval-all '[select(.kind == "Secret" and .metadata.name == "kuberploy-git-ssh" and .metadata.namespace == "kuberploy-system")] | length' "${kp_tmp}/adopted-control-plane-secrets.yaml" | tail -1)" == "1" ]]
[[ "$(yq eval-all '[select(.kind == "Secret" and .metadata.name != "kuberploy-git-ssh")] | length' "${kp_tmp}/adopted-control-plane-secrets.yaml" | tail -1)" == "0" ]]
...
kp_expect_reject "unprefixed source revision" --set-string source.valuesRevision=0.1.0-rc.413
kp_expect_reject "missing package version" --set-string components.postgresql.expectedPackageVersion=
kp_stale_version_output="$(helm template stale-package-version "${kp_chart}" --namespace kuberploy-system -f "${kp_managed}" --set-string components.postgresql.expectedPackageVersion=0.1.0-rc.258 2>&1 || true)"
grep -q 'must match installer chart version 0.1.0-rc.413' <<<"${kp_stale_version_output}"
kp_expect_reject "unsupported adopted monitoring" --set components.postgresql.enabled=false --set components.postgresql.mode=disabled --set-string components.postgresql.expectedPackageVersion= --set-json components.postgresql.valueFiles=[] --set components.monitoring.enabled=true --set components.monitoring.mode=adopted --set components.monitoring.adoptionConfirmed=true --set-string components.monitoring.expectedPackageVersion=0.1.0-rc.413
...
kp_expect_reject "control plane without explicit bootstrap token authority" --set components.controlPlane.enabled=true --set components.controlPlane.mode=managed --set-string components.controlPlane.expectedPackageVersion=0.1.0-rc.413 --set components.valkey.enabled=true --set components.valkey.mode=managed --set-string components.valkey.expectedPackageVersion=0.1.0-rc.413
helm template functional-generated-token "${kp_chart}" --namespace kuberploy-system -f "${kp_managed}" --set bootstrap.controlPlaneToken.mode=generated --set components.controlPlane.enabled=true --set components.controlPlane.mode=managed --set-string components.controlPlane.expectedPackageVersion=0.1.0-rc.413 --set components.valkey.enabled=true --set components.valkey.mode=managed --set-string components.valkey.expectedPackageVersion=0.1.0-rc.413 >/dev/null
helm template functional-precreated-token "${kp_chart}" --namespace kuberploy-system -f "${kp_managed}" --set bootstrap.controlPlaneToken.mode=precreated --set-string bootstrap.controlPlaneToken.kubeAPIServerCIDRs[0]=10.43.0.1/32 --set components.controlPlane.enabled=true --set components.controlPlane.mode=managed --set-string components.controlPlane.expectedPackageVersion=0.1.0-rc.413 --set components.valkey.enabled=true --set components.valkey.mode=managed --set-string components.valkey.expectedPackageVersion=0.1.0-rc.413 >/dev/null
helm template functional-edge "${kp_chart}" --namespace kuberploy-system -f "${kp_managed}" --set components.postgresql.enabled=false --set components.postgresql.mode=disabled --set-string components.postgresql.expectedPackageVersion= --set-json components.postgresql.valueFiles=[] --set components.edge.enabled=true --set components.edge.mode=managed --set-string components.edge.expectedPackageVersion=0.1.0-rc.413 >/dev/null
...
kp_expect_reject "GitHub without builder component" "${kp_platform_args[@]}" --set publicEndpoint.tls.enabled=true --set-string publicEndpoint.tls.secretName=kuberploy-platform-tls --set-string publicEndpoint.tls.clusterIssuerName=kuberploy-letsencrypt-production --set-string publicEndpoint.tls.accountEmail=platform@example.com --set integrations.github.enabled=true --set integrations.github.appID=123456 --set-string integrations.github.clientID=Iv1_KuberployClient --set-string integrations.github.appSlug=kuberploy-test --set-string integrations.github.secretName=kuberploy-github-app --set-string integrations.github.controlPlaneEgressCIDRs[0]=192.0.2.10/32 --set-string integrations.github.sourceEgressCIDRs[0]=192.0.2.11/32 --set-string integrations.github.registryEgressCIDRs[0]=192.0.2.12/32
...
kp_expect_reject "managed registry without registry integration" --set components.registry.enabled=true --set components.registry.mode=managed --set-string components.registry.expectedPackageVersion=0.1.0-rc.413
