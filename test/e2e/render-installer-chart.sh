...
kp_expected_runtime_lock="sha256:$(printf 'kuberploy-runtime-lock-v1|0.1.0-rc.355|%s' "${kp_expected_runtime_digest}" | shasum -a 256 | cut -d ' ' -f 1)"
...
text = text.replace(
    "annotations:\n",
    "annotations:\n"
    '  kuberploy.io/runtime-chart-version: "0.1.0-rc.355"\n'
...
[[ -f "${kp_chart}/charts/kuberploy-argocd-0.1.0-rc.355.tgz" ]]
[[ -f "${kp_chart}/charts/kuberploy-valkey-0.1.0-rc.355.tgz" ]]
python3 - "${kp_chart}" <<'PY'
import hashlib
import re
import sys
...
helm template kuberploy-installer "${kp_chart}" --namespace kuberploy-system -f "${kp_managed}" --is-upgrade \
	--set-string source.valuesRevision=v0.1.0-rc.355-upgrade \
	--set-string components.postgresql.expectedPackageVersion=0.1.0-rc.355 \
...
[[ "$(yq eval-all 'select(.kind == "Application") | .spec.sources[0].targetRevision' "${kp_tmp}/managed.yaml")" == "0.1.0-rc.355" ]]
...
[[ "$(yq eval-all 'select(.kind == "Application") | .spec.sources[1].targetRevision' "${kp_tmp}/managed.yaml")" == "v0.1.0-rc.355" ]]
...
[[ "$(yq eval-all 'select(.kind == "Job" and .metadata.name == "kuberploy-installer-application-health") | [.spec.template.spec.containers[].args[1]] | join(",")' "${kp_tmp}/managed.yaml")" == '--for=jsonpath={.spec.sources[0].targetRevision}=0.1.0-rc.355,--for=jsonpath={.status.sync.revisions[0]}=0.1.0-rc.355,--for=jsonpath={.status.sync.status}=Synced,--for=jsonpath={.status.health.status}=Healthy' ]]
...
[[ "$(yq eval-all -r 'select(.kind == "ConfigMap" and .metadata.labels."app.kubernetes.io/component" == "application-inventory") | .data."inventory.json"' "${kp_tmp}/managed-upgrade.yaml" | jq -r '.applications[0].targetPackageVersion')" == "0.1.0-rc.355" ]]
...
[[ "$(yq eval-all 'select(.kind == "Job" and .metadata.name == "kuberploy-installer-application-health") | [.spec.template.spec.containers[].args[1]] | join(",")' "${kp_tmp}/managed-upgrade.yaml")" == '--for=jsonpath={.spec.sources[0].targetRevision}=0.1.0-rc.355,--for=jsonpath={.status.sync.revisions[0]}=0.1.0-rc.355,--for=jsonpath={.status.sync.status}=Synced,--for=jsonpath={.status.health.status}=Healthy' ]]
...
[[ "$(yq eval-all 'select(.kind == "Job" and .metadata.name == "kuberploy-installer-application-health") | [.spec.template.spec.containers[].args[1]] | join(",")' "${kp_tmp}/managed-rollback.yaml")" == '--for=jsonpath={.spec.sources[0].targetRevision}=0.1.0-rc.355,--for=jsonpath={.status.sync.revisions[0]}=0.1.0-rc.355,--for=jsonpath={.status.sync.status}=Synced,--for=jsonpath={.status.health.status}=Healthy' ]]
...
helm template kuberploy-installer "${kp_chart}" --namespace kuberploy-system -f "${kp_managed}" \
  ...
  --set-string components.controlPlane.expectedPackageVersion=0.1.0-rc.355 \
  --set components.valkey.enabled=true \
  --set components.valkey.mode=managed \
  --set-string components.valkey.expectedPackageVersion=0.1.0-rc.355 \
...
for kp_component in controlPlane postgresql valkey edge certManager externalDNS externalSecrets sealedSecrets monitoring builder registry; do
  ...
  kp_all_args+=(--set-string "components.${kp_component}.expectedPackageVersion=0.1.0-rc.355")
done
...
[[ "$(yq eval-all 'select(.kind == "Job" and .metadata.name == "kuberploy-installer-application-health") | [.spec.template.spec.containers[].args[1] | select(. == "--for=jsonpath={.spec.sources[0].targetRevision}=0.1.0-rc.355")] | length' "${kp_tmp}/all-components.yaml")" == "10" ]]
[[ "$(yq eval-all 'select(.kind == "Job" and .metadata.name == "kuberploy-installer-application-health") | [.spec.template.spec.containers[].args[1] | select(. == "--for=jsonpath={.status.sync.revisions[0]}=0.1.0-rc.355")] | length' "${kp_tmp}/all-components.yaml")" == "10" ]]
...
kp_platform_args=(
  ...
  --set-string components.edge.expectedPackageVersion=0.1.0-rc.355
  --set components.certManager.enabled=true
  --set components.certManager.mode=managed
  --set-string components.certManager.expectedPackageVersion=0.1.0-rc.355
  --set components.sealedSecrets.enabled=true
  --set components.sealedSecrets.mode=managed
  --set-string components.sealedSecrets.expectedPackageVersion=0.1.0-rc.355
  --set components.monitoring.enabled=true
  --set components.monitoring.mode=managed
  --set-string components.monitoring.expectedPackageVersion=0.1.0-rc.355
...
kp_github_args=(
  ...
  --set-string components.builder.expectedPackageVersion=0.1.0-rc.355
...
[[ "$(yq eval-all 'select(.kind == "Application" and .metadata.name == "kuberploy-control-plane") | .spec.sources[0].helm.valuesObject.config.gitProjection.chartVersion' "${kp_tmp}/github-platform.yaml")" == "0.1.0-rc.355" ]]
[[ "$(yq eval-all 'select(.kind == "Application" and .metadata.name == "kuberploy-control-plane") | .spec.sources[0].helm.valuesObject.config.argoDesiredState.runtimeChartVersion' "${kp_tmp}/github-platform.yaml")" == "0.1.0-rc.355" ]]
...
[[ "$(yq eval-all 'select(.kind == "AppProject" and .metadata.name == "kuberploy-control-plane") | [.spec.destinations[].namespace] | sort | join(",")' "${kp_tmp}/runtime-secrets-platform.yaml")" == "argocd,cert-manager,kp-example-development,kp-example-production,kuberploy-build-dind,kuberploy-helm-renderer,kuberploy-monitoring,kuberploy-system" ]]
...
kp_registry_pull_args=(
  ...
  --set-string components.registry.expectedPackageVersion=0.1.0-rc.355
...
[[ "$(yq eval-all 'select(.kind == "Application" and .metadata.name == "kuberploy-builder") | .spec.sources[0].targetRevision' "${kp_tmp}/github-platform.yaml")" == "0.1.0-rc.355" ]]
...
helm template kuberploy "${kp_root}/charts/kuberploy" --namespace kuberploy-system \
  ...
  --set-string builder.builderAgentImage=ghcr.io/kuberploy/kuberploy-builder-agent:0.1.0-rc.355 \
...
helm template kuberploy-installer "${kp_chart}" --namespace kuberploy-system -f "${kp_adopted}" \
  ...
  --set components.controlPlane.mode=managed --set-string components.controlPlane.expectedPackageVersion=0.1.0-rc.355 \
  --set components.postgresql.enabled=true --set components.postgresql.mode=adopted --set components.postgresql.adoptionConfirmed=true --set-string components.postgresql.expectedPackageVersion=0.1.0-rc.355 \
  --set components.valkey.enabled=true --set components.valkey.mode=adopted --set components.valkey.adoptionConfirmed=true --set-string components.valkey.expectedPackageVersion=0.1.0-rc.355 \
...
kp_expect_reject "unprefixed source revision" --set-string source.valuesRevision=0.1.0-rc.355
...
grep -q 'must match installer chart version 0.1.0-rc.355' <<<"${kp_stale_version_output}"
kp_expect_reject "unsupported adopted monitoring" --set components.postgresql.enabled=false --set components.postgresql.mode=disabled --set-string components.postgresql.expectedPackageVersion= --set-json components.postgresql.valueFiles=[] --set components.monitoring.enabled=true --set components.monitoring.mode=adopted --set components.monitoring.adoptionConfirmed=true --set-string components.monitoring.expectedPackageVersion=0.1.0-rc.355
...
kp_expect_reject "control plane without explicit bootstrap token authority" --set components.controlPlane.enabled=true --set components.controlPlane.mode=managed --set-string components.controlPlane.expectedPackageVersion=0.1.0-rc.355 --set components.valkey.enabled=true --set components.valkey.mode=managed --set-string components.valkey.expectedPackageVersion=0.1.0-rc.355
helm template functional-generated-token "${kp_chart}" --namespace kuberploy-system -f "${kp_managed}" --set bootstrap.controlPlaneToken.mode=generated --set components.controlPlane.enabled=true --set components.controlPlane.mode=managed --set-string components.controlPlane.expectedPackageVersion=0.1.0-rc.355 --set components.valkey.enabled=true --set components.valkey.mode=managed --set-string components.valkey.expectedPackageVersion=0.1.0-rc.355 >/dev/null
helm template functional-precreated-token "${kp_chart}" --namespace kuberploy-system -f "${kp_managed}" --set bootstrap.controlPlaneToken.mode=precreated --set-string bootstrap.controlPlaneToken.kubeAPIServerCIDRs[0]=10.43.0.1/32 --set components.controlPlane.enabled=true --set components.controlPlane.mode=managed --set-string components.controlPlane.expectedPackageVersion=0.1.0-rc.355 --set components.valkey.enabled=true --set components.valkey.mode=managed --set-string components.valkey.expectedPackageVersion=0.1.0-rc.355 >/dev/null
helm template functional-edge "${kp_chart}" --namespace kuberploy-system -f "${kp_managed}" --set components.postgresql.enabled=false --set components.postgresql.mode=disabled --set-string components.postgresql.expectedPackageVersion= --set-json components.postgresql.valueFiles=[] --set components.edge.enabled=true --set components.edge.mode=managed --set-string components.edge.expectedPackageVersion=0.1.0-rc.355 >/dev/null
...
kp_expect_reject "managed registry without registry integration" --set components.registry.enabled=true --set components.registry.mode=managed --set-string components.registry.expectedPackageVersion=0.1.0-rc.355
