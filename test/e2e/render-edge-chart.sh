...
[[ "$(yq eval-all -o=json -I=0 'select(.kind == "NetworkPolicy" and .metadata.name == "edge-traefik") | .spec.egress[] | select(.to[0].podSelector.matchLabels."app.kubernetes.io/name" == "kuberploy-registry")' "${kp_tmp}/edge.yaml" | jq -cS .)" == '{"ports":[{"port":5000,"protocol":"TCP"}],"to":[{"podSelector":{"matchLabels":{"app.kubernetes.io/component":"registry","app.kubernetes.io/name":"kuberploy-registry"}}}]}' ]]
