...
if [[ " $* " == *' template '* ]]; then
  for kp_name in control-plane edge; do
    printf '%s\n' '---' 'apiVersion: argoproj.io/v1alpha1' 'kind: Application' \
      ...
      '  annotations:' \
      '    kuberploy.io/expected-package-digest: "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"' \
      'spec:' '  source:' \
...
grep -F 'helm|upgrade --install kuberploy-qualification /Users/jojo/kuberploy/charts/kuberploy-installer --namespace argocd' \
