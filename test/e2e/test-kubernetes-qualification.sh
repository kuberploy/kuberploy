...
if [[ "${1:-}" == "config" && "${2:-}" == "get-contexts" ]]; then
  ...
  printf '%s\n' '{"metadata":{"name":"kp-h-33333333333343338333333333333333","labels":{"app.kubernetes.io/component":"helm-application","kuberploy.io/application-id":"33333333-3333-4333-8333-333333333333","kuberploy.io/environment-id":"22222222-2222-4222-8222-222222222222"},"annotations":{"kuberploy.io/helm-revision-id":"41414141-4141-4141-8141-414141414141"}},"spec":{"source":{"targetRevision":"1.0.0"}},"status":{"sync":{"status":"Synced"},"health":{"status":"Healthy"},"resources":[{"kind":"Deployment","status":"Synced","health":{"status":"Healthy"}}]}}'
...
if [[ " $* " == *' template '* ]]; then
  for kp_name in control-plane edge; do
    printf '%s\n' '---' 'apiVersion: argoproj.io/v1alpha1' 'kind: Application' \
      ...
      '  annotations:' \
      '    kuberploy.io/expected-package-version: "0.1.0-rc.429"' \
      'spec:' '  source:' \
...
if [[ "${kp_method}" == "POST" && "${kp_url}" == */v1/auth/bootstrap ]]; then
  ...
  printf '%s\n' '{"id":"41414141-4141-4141-8141-414141414141","generation":1,"releaseName":"qualification","action":"deploy","desiredEnabled":true,"source":{"kind":"oci","repositoryUrl":"registry.fixture.test/kuberploy","chart":"qualification","targetRevision":"1.0.0"},"valuesYaml":"replicaCount: 1\n","valuesDigest":"sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","state":"applied","requestId":"qualification","createdAt":"2026-08-09T00:00:00Z","updatedAt":"2026-08-09T00:00:00Z"}' >"${kp_output}"; printf '202'
elif [[ "${kp_method}" == "GET" && "${kp_url}" == */helm/release ]]; then
  printf '%s\n' '{"id":"41414141-4141-4141-8141-414141414141","generation":1,"releaseName":"qualification","action":"deploy","desiredEnabled":true,"source":{"kind":"oci","repositoryUrl":"registry.fixture.test/kuberploy","chart":"qualification","targetRevision":"1.0.0"},"valuesYaml":"replicaCount: 1\n","valuesDigest":"sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","state":"applied","requestId":"qualification","createdAt":"2026-08-09T00:00:00Z","updatedAt":"2026-08-09T00:00:00Z"}' >"${kp_output}"; printf '200'
...
kp_scenario="$(jq -c --arg digest "${kp_teardown_key_digest}" \
  '.teardown.publicKeySHA256=$digest |
   ...
   .workflow.helm={source:{kind:"oci",repositoryUrl:"registry.fixture.test/kuberploy",chart:"qualification",targetRevision:"1.0.0"},valuesYaml:"replicaCount: 1\n"}' \
  <<<"${kp_scenario}")"
...
jq -e '.mutation == "github-webhook-build-cancel-cache-fault-auto-deploy-promotion-and-direct-helm" and
