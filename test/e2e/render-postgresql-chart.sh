...
[[ "$(yq eval-all -o=json -I=0 'select(.kind == "NetworkPolicy" and .metadata.name == "kuberploy-postgresql-private-egress") | .spec.podSelector.matchLabels' "${kp_tmp}/managed.yaml" | jq -cS .)" == '{"app.kubernetes.io/instance":"postgresql","app.kubernetes.io/name":"kuberploy-postgresql"}' ]]
