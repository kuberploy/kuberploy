...
[[ "$(yq eval-all 'select(.kind == "Service" and .metadata.name == "authenticated-kuberploy-registry") | .metadata.annotations."traefik.ingress.kubernetes.io/service.serversscheme"' "${kp_auth_render}")" == "https" ]]
[[ "$(yq eval-all 'select(.kind == "Service" and .metadata.name == "authenticated-kuberploy-registry") | .metadata.annotations."traefik.ingress.kubernetes.io/service.serverstransport"' "${kp_auth_render}")" == "kuberploy-registry-authenticated-kuberploy-registry-backend@kubernetescrd" ]]
