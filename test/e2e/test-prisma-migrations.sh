...
# Exercise the ordered 003 -> 019 production upgrade, not only a fresh apply.
...
[[ "$(docker exec "${kp_postgres}" psql --username postgres --dbname upgrade --tuples-only --no-align --command \
    "SELECT count(*) FROM _prisma_migrations WHERE finished_at IS NOT NULL AND rolled_back_at IS NULL")" == "19" ]]
[[ "$(docker exec "${kp_postgres}" psql --username postgres --dbname upgrade --tuples-only --no-align --command \
...
printf 'Prisma migration image delayed database wait, fresh and 003-to-019 apply, RC171 recovery, self-upgrade retirement, declarative drift, old-writer fencing, personal/team scope authority, idempotency, native authority, and legacy rejection passed\n'
