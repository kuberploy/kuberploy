...
[[ "$(yq eval-all 'select(.kind == "NetworkPolicy" and .metadata.name == "platform-default-upgrade") | ([.spec.egress[]?] | length) == 0' "${kp_tmp}/platform-default.yaml")" == "true" ]]
...
yq '.config.githubApp.enabled = true |
    ...
    .builder.builderAgentImage = "ghcr.io/kuberploy/kuberploy-builder-agent:0.1.0-rc.59" |
...
yq '.config.gitProjection.enabled = true | .config.gitProjection.chartVersion = "0.1.0-rc.59"' \
...
[[ "$(yq eval-all 'select(.kind == "ConfigMap") | .data.KUBERPLOY_GIT_PROJECTION_CHART_VERSION' "${kp_tmp}/git-projection.yaml")" == "0.1.0-rc.59" ]]
...
[[ "$(yq eval-all -o=json -I=0 'select(.kind == "Role" and .metadata.labels."app.kubernetes.io/component" == "argo-desired-state") | .rules' "${kp_tmp}/argo-desired-state.yaml")" == '[{"apiGroups":["argoproj.io"],"resources":["applications"],"resourceNames":["kuberploy-platform-root"],"verbs":["get"]},{"apiGroups":[""],"resources":["secrets"],"verbs":["create","patch","delete"]}]' ]]
...
[[ "$(yq eval-all -o=json 'select(.kind == "ConfigMap" and .data.KUBERPLOY_HELM_PACKAGE_CACHE_BYTES != null)' "${kp_tmp}/helm-applications.yaml" | jq '(.data.KUBERPLOY_HELM_PACKAGE_CACHE_BYTES | tonumber) == 67108864')" == "true" ]]
for kp_component in api worker; do
...
yq '.config.gitProjection.enabled = true | .config.gitProjection.chartVersion = "0.1.0-rc.59"' "${kp_root}/test/e2e/fixtures/platform-values.yaml" > "${kp_tmp}/git-projection-no-github.yaml"
...
if helm template invalid-github-builds "${kp_root}/charts/kuberploy" \
  ...
  printf 'platform chart accepted a shared registry/API node without explicit acknowledgement\n' >&2
  exit 1
fi
yq '.builder.networkPolicy.registryEgressCIDRs = [.networkPolicy.kubeAPIServerCIDRs[0]] |
    .builder.networkPolicy.allowSharedNodeEndpoint = true' \
  "${kp_tmp}/github-build-values.yaml" > "${kp_tmp}/github-build-shared-node.yaml"
helm template github-builds "${kp_root}/charts/kuberploy" \
  --namespace kuberploy-e2e-render -f "${kp_tmp}/github-build-shared-node.yaml" >/dev/null
yq '.builder.networkPolicy.sourceEgressCIDRs = [.networkPolicy.kubeAPIServerCIDRs[0]] |
    .builder.networkPolicy.allowSharedNodeEndpoint = true' \
  "${kp_tmp}/github-build-values.yaml" > "${kp_tmp}/github-build-shared-source-rejected.yaml"
if helm template invalid-github-builds "${kp_root}/charts/kuberploy" \
  --namespace kuberploy-e2e-render -f "${kp_tmp}/github-build-shared-source-rejected.yaml" >/dev/null 2>&1; then
  printf 'platform chart accepted source egress to the Kubernetes API node\n' >&2
  exit 1
fi
...
[[ "$(yq eval-all 'select(.kind == "Job" and .metadata.labels."app.kubernetes.io/component" == "migration") | .spec.template.spec.containers[0].image' "${kp_tmp}/upgrade-job-render.yaml")" == "ghcr.io/kuberploy/kuberploy-migration:0.1.0-rc.59" ]]
