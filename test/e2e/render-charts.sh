...
yq '.config.githubApp.enabled = true |
    ...
    .builder.builderAgentImage = "ghcr.io/kuberploy/kuberploy-builder-agent:0.1.0-rc.232" |
...
yq '.config.gitProjection.enabled = true | .config.gitProjection.chartVersion = "0.1.0-rc.232"' \
...
[[ "$(yq eval-all 'select(.kind == "ConfigMap") | .data.KUBERPLOY_GIT_PROJECTION_CHART_VERSION' "${kp_tmp}/git-projection.yaml")" == "0.1.0-rc.232" ]]
...
yq '.config.gitProjection.enabled = true | .config.gitProjection.chartVersion = "0.1.0-rc.232"' "${kp_root}/test/e2e/fixtures/platform-values.yaml" > "${kp_tmp}/git-projection-no-github.yaml"
...
[[ "$(yq eval-all 'select(.kind == "Job" and .metadata.labels."app.kubernetes.io/component" == "migration") | .spec.template.spec.containers[0].image' "${kp_tmp}/upgrade-job-render.yaml")" == "ghcr.io/kuberploy/kuberploy-migration:0.1.0-rc.232" ]]
