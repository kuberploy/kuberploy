...
helm lint "${kp_root}/deploy/orbstack/local-git"
...
[[ "$(yq eval-all 'select(.kind == "NetworkPolicy" and .metadata.name == "platform-default-upgrade") | .spec.egress == []' "${kp_tmp}/platform-default.yaml")" == "true" ]]
...
[[ "$(yq eval-all '[select(.kind == "Deployment" and (.metadata.labels."app.kubernetes.io/component" == "api" or .metadata.labels."app.kubernetes.io/component" == "worker")) | .spec.template.spec.containers[0].env[] | select((.name == "KUBERPLOY_VALKEY_USERNAME" and .valueFrom.secretKeyRef.key == "username" or .name == "KUBERPLOY_VALKEY_PASSWORD" and .valueFrom.secretKeyRef.key == "password") and .valueFrom.secretKeyRef.name == "external-valkey" and .valueFrom.secretKeyRef.optional == true)] | length' "${kp_tmp}/platform.yaml" | tail -1)" == "4" ]]
...
[[ "$(yq eval-all 'select(.kind == "Role" and (.metadata.name | test("managed-monitoring-observer$"))) | .rules == [{"apiGroups":[""],"resources":["configmaps"],"resourceNames":["monitoring-monitoring-profile"],"verbs":["get"]},{"apiGroups":["apps"],"resources":["deployments"],"resourceNames":["kuberploy-prometheus-operator"],"verbs":["get"]},{"apiGroups":["monitoring.coreos.com"],"resources":["prometheusrules"],"resourceNames":["monitoring-service-recording-rules"],"verbs":["get"]}]' "${kp_tmp}/managed-monitoring.yaml")" == "true" ]]
[[ "$(yq eval-all 'select(.kind == "RoleBinding" and (.metadata.name | test("managed-monitoring-observer$"))) | .subjects == [{"kind":"ServiceAccount","name":"managed-monitoring-api","namespace":"kuberploy-e2e-render"}]' "${kp_tmp}/managed-monitoring.yaml")" == "true" ]]
...
[[ "$(yq eval-all '[select(.kind == "NetworkPolicy" and (.metadata.labels."app.kubernetes.io/component" == "api" or .metadata.labels."app.kubernetes.io/component" == "worker")) | .spec.egress[] | select(.to[0].namespaceSelector.matchLabels."kubernetes.io/metadata.name" == "kuberploy-postgresql" and .to[0].podSelector.matchLabels."app.kubernetes.io/name" == "kuberploy-postgresql" and (.ports | length) == 1 and .ports[0].port == 5432)] | length' "${kp_tmp}/platform.yaml" | tail -1)" == "2" ]]
[[ "$(yq eval-all '[select(.kind == "NetworkPolicy" and (.metadata.labels."app.kubernetes.io/component" == "api" or .metadata.labels."app.kubernetes.io/component" == "worker")) | .spec.egress[] | select(.to[0].namespaceSelector.matchLabels."kubernetes.io/metadata.name" == "kuberploy-valkey" and .to[0].podSelector.matchLabels."app.kubernetes.io/name" == "valkey" and (.ports | length) == 1 and .ports[0].port == 6379)] | length' "${kp_tmp}/platform.yaml" | tail -1)" == "2" ]]
[[ "$(yq eval-all '[select(.kind == "NetworkPolicy" and (.metadata.labels."app.kubernetes.io/component" == "api" or .metadata.labels."app.kubernetes.io/component" == "worker" or .metadata.labels."app.kubernetes.io/component" == "upgrade")) | .spec.egress[] | select(.to[0].ipBlock.cidr == "10.43.0.1/32" and (.ports | length) == 2 and .ports[0].port == 443 and .ports[1].port == 6443)] | length' "${kp_tmp}/platform.yaml" | tail -1)" == "3" ]]
...
[[ "$(yq eval-all '[select(.kind == "NetworkPolicy" and .metadata.labels."app.kubernetes.io/component" == "upgrade") | .spec.egress[] | select(.to[0].ipBlock.cidr == "192.0.2.10/32" and (.ports | length) == 1 and .ports[0].port == 443)] | length' "${kp_tmp}/platform.yaml" | tail -1)" == "1" ]]
...
if [[ "$(yq eval-all '[select(.kind == "NetworkPolicy") | .spec.egress[]? | select(has("ports") | not)] | length' "${kp_tmp}/platform.yaml" | tail -1)" != "0" ]]; then
...
[[ "$(yq eval-all '[select(.kind == "NetworkPolicy" and (.metadata.labels."app.kubernetes.io/component" == "worker" or .metadata.labels."app.kubernetes.io/component" == "upgrade")) | .spec.egress[] | select(.to[0].ipBlock.cidr == "10.43.0.1/32")] | length' "${kp_tmp}/no-runtime-observer.yaml" | tail -1)" == "2" ]]
...
[[ "$(yq eval-all 'select(.kind == "Role" and (.metadata.name | test("-argo-observer$"))) | .rules == [{"apiGroups":["argoproj.io"],"resources":["applications"],"verbs":["list"]}]' "${kp_tmp}/argo-observation.yaml")" == "true" ]]
[[ "$(yq eval-all 'select(.kind == "RoleBinding" and (.metadata.name | test("-argo-observer$"))) | .subjects == [{"kind":"ServiceAccount","name":"argo-observation-worker","namespace":"kuberploy-e2e-render"}]' "${kp_tmp}/argo-observation.yaml")" == "true" ]]
...
yq '.components.worker.image.reference = "ghcr.io/kuberploy/kuberploy-worker@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" |
    ...
    .config.managedRegistry.endpoint = "http://kuberploy-registry.kuberploy-registry.svc.cluster.local:5000" |
    ...
    .config.managedRegistry.namespace = "kuberploy-registry" |
...
[[ "$(yq eval-all 'select(.kind == "ServiceAccount" and (.metadata.name | test("registry-maintenance$"))) | .metadata.namespace' "${kp_tmp}/managed-registry.yaml")" == "kuberploy-registry" ]]
...
[[ "$(yq eval-all 'select(.kind == "Role" and (.metadata.name | test("registry-maintenance-controller$"))) | [.rules[] | select(.resources == ["secrets"] or .resources == ["pods/log"])] | length' "${kp_tmp}/managed-registry.yaml")" == "0" ]]
[[ "$(yq eval-all 'select(.kind == "Role" and (.metadata.name | test("registry-maintenance-controller$"))) | [.rules[] | select(.resources == ["deployments"] and .resourceNames == ["kuberploy-registry"] and .verbs == ["get"])] | length' "${kp_tmp}/managed-registry.yaml")" == "1" ]]
[[ "$(yq eval-all 'select(.kind == "NetworkPolicy" and (.metadata.name | test("registry-maintenance-deny-network$"))) | .metadata.namespace == "kuberploy-registry" and .spec.ingress == [] and .spec.egress == []' "${kp_tmp}/managed-registry.yaml")" == "true" ]]
[[ "$(yq eval-all '[select(.kind == "NetworkPolicy" and .metadata.labels."app.kubernetes.io/component" == "worker") | .spec.egress[] | select(.to[0].namespaceSelector.matchLabels."kubernetes.io/metadata.name" == "kuberploy-registry" and .to[0].podSelector.matchLabels."app.kubernetes.io/name" == "kuberploy-registry" and .ports == [{"port":5000,"protocol":"TCP"}])] | length' "${kp_tmp}/managed-registry.yaml" | tail -1)" == "1" ]]
...
yq '.config.githubApp.enabled = true |
    .config.githubApp.appID = 4543722 |
...
[[ "$(yq eval-all 'select(.kind == "ConfigMap") | .data.KUBERPLOY_GITHUB_APP_ID' "${kp_tmp}/github-builds.yaml")" == "4543722" ]]
...
[[ "$(yq eval-all '[select(.kind == "Deployment" and (.metadata.labels."app.kubernetes.io/component" == "api" or .metadata.labels."app.kubernetes.io/component" == "worker")) | .spec.template.spec.containers[0].env[] | select(.name == "KUBERPLOY_BUILDER_AGENT_IMAGE" or .name == "KUBERPLOY_BUILDER_SOURCE_EGRESS_CIDRS" or .name == "KUBERPLOY_BUILDER_REGISTRY_EGRESS_CIDRS")] | length' "${kp_tmp}/github-builds.yaml" | tail -1)" == "6" ]]
...
[[ "$(yq eval-all 'select(.kind == "Role" and (.metadata.name | test("-build-logs$"))) | .rules == [{"apiGroups":["batch"],"resources":["jobs"],"verbs":["get"]},{"apiGroups":[""],"resources":["pods"],"verbs":["get","list"]},{"apiGroups":[""],"resources":["pods/log"],"verbs":["get"]}]' "${kp_tmp}/build-logs.yaml")" == "true" ]]
[[ "$(yq eval-all 'select(.kind == "RoleBinding" and (.metadata.name | test("-build-logs$"))) | .subjects == [{"kind":"ServiceAccount","name":"github-builds-api","namespace":"kuberploy-e2e-render"}]' "${kp_tmp}/build-logs.yaml")" == "true" ]]
...
for kp_component in api worker; do
  ...
  [[ "$(kp_component="${kp_component}" yq eval-all '[select(.kind == "Deployment" and .metadata.labels."app.kubernetes.io/component" == strenv(kp_component)) | .spec.template.spec.containers[0].env[] | select(.name == "KUBERPLOY_ENVIRONMENT_FOUNDATION_ENABLED" or .name == "KUBERPLOY_ENVIRONMENT_FOUNDATION_PLATFORM_BINDING_ID" or .name == "KUBERPLOY_ENVIRONMENT_FOUNDATION_PSA_VERSION" or .name == "KUBERPLOY_ENVIRONMENT_FOUNDATION_POLL_INTERVAL_SECONDS")] | length' "${kp_tmp}/argo-desired-state.yaml" | tail -1)" == "4" ]]
done
...
[[ "$(yq eval-all 'select(.kind == "Role" and .metadata.labels."app.kubernetes.io/component" == "argo-desired-state") | .metadata.namespace == "kuberploy-e2e-render" and .rules == [{"apiGroups":["argoproj.io"],"resources":["applications"],"resourceNames":["kuberploy-platform-root"],"verbs":["get"]},{"apiGroups":[""],"resources":["secrets"],"verbs":["create","patch","delete"]}]' "${kp_tmp}/argo-desired-state.yaml")" == "true" ]]
[[ "$(yq eval-all 'select(.kind == "RoleBinding" and .metadata.labels."app.kubernetes.io/component" == "argo-desired-state") | .subjects == [{"kind":"ServiceAccount","name":"argo-desired-worker","namespace":"kuberploy-e2e-render"}]' "${kp_tmp}/argo-desired-state.yaml")" == "true" ]]
[[ "$(yq eval-all '[select(.kind == "Role" and .metadata.labels."app.kubernetes.io/component" == "argo-desired-state") | .rules[] | select(.resources == ["secrets"]) | .verbs[] | select(. == "get" or . == "list" or . == "watch" or . == "update")] | length' "${kp_tmp}/argo-desired-state.yaml" | tail -1)" == "0" ]]
...
[[ "$(yq eval-all 'select(.kind == "ValidatingAdmissionPolicy" and .metadata.name == strenv(kp_argo_admission_name)) | .spec.failurePolicy == "Fail" and .spec.matchConstraints.matchPolicy == "Exact" and .spec.matchConstraints.resourceRules == [{"apiGroups":[""],"apiVersions":["v1"],"operations":["CREATE","UPDATE","DELETE"],"resources":["secrets"],"scope":"Namespaced"}] and (.spec.matchConditions | length) == 2 and (.spec.validations | length) == 4' "${kp_tmp}/argo-desired-state.yaml")" == "true" ]]
[[ "$(yq eval-all 'select(.kind == "ValidatingAdmissionPolicy" and .metadata.name == strenv(kp_argo_admission_name)) | [.spec.matchConditions[].expression] | join(",")' "${kp_tmp}/argo-desired-state.yaml")" == "request.userInfo.username == 'system:serviceaccount:kuberploy-e2e-render:argo-desired-worker',request.namespace == 'kuberploy-e2e-render'" ]]
[[ "$(yq eval-all 'select(.kind == "ValidatingAdmissionPolicy" and .metadata.name == strenv(kp_argo_admission_name)) | [.spec.validations[].expression] | join("\n") | contains("object.metadata.name == 'kuberploy-repo-' + object.metadata.labels['kuberploy.io/git-binding-id'].replace('-', '')") and contains("object.metadata.labels.size() == 4") and contains("object.metadata.annotations.size() == 2") and contains("object.data.size() == 5") and contains("githubAppPrivateKey") and contains("oldObject.metadata.name == 'kuberploy-repo-'") and contains("request.operation != 'DELETE'")' "${kp_tmp}/argo-desired-state.yaml")" == "true" ]]
[[ "$(yq eval-all 'select(.kind == "ValidatingAdmissionPolicyBinding" and .metadata.name == strenv(kp_argo_admission_name)) | .spec.policyName == strenv(kp_argo_admission_name) and .spec.validationActions == ["Deny"] and .spec.matchResources.matchPolicy == "Exact" and .spec.matchResources.namespaceSelector == {"matchLabels":{"kubernetes.io/metadata.name":"kuberploy-e2e-render"}}' "${kp_tmp}/argo-desired-state.yaml")" == "true" ]]
...
[[ "$(yq eval-all 'select(.kind == "RoleBinding" and (.metadata.name | test("-helm-renderer$"))) | .subjects == [{"kind":"ServiceAccount","name":"helm-applications-worker","namespace":"kuberploy-e2e-render"}]' "${kp_tmp}/helm-applications.yaml")" == "true" ]]
[[ "$(yq eval-all 'select(.kind == "NetworkPolicy" and (.metadata.name | test("-helm-renderer-default-deny$"))) | .metadata.namespace == "kuberploy-helm-renderer" and .spec.ingress == [] and .spec.egress == []' "${kp_tmp}/helm-applications.yaml")" == "true" ]]
...
[[ "$(yq eval-all '[select(.kind == "Deployment" and (.metadata.labels."app.kubernetes.io/component" == "api" or .metadata.labels."app.kubernetes.io/component" == "worker")) | .spec.template.spec.volumes[] | select(.name == "runtime-registry-pull-credentials") | [.projected.sources[] | .secret.name + ":" + .secret.items[0].key + ":" + .secret.items[0].path] | join(",")] | unique | join("|")' "${kp_tmp}/runtime-registry-pulls.yaml" | tail -1)" == "pull-source-a:.dockerconfigjson:managed-a/dockerconfigjson,pull-source-b:config.json:managed-b/dockerconfigjson" ]]
...
for kp_runtime_pull_namespace in apps-production apps-staging; do
  ...
  [[ "$(kp_runtime_pull_namespace="${kp_runtime_pull_namespace}" kp_runtime_pull_names="${kp_runtime_pull_names}" yq eval-all 'select(.kind == "Role" and .metadata.labels."app.kubernetes.io/component" == "runtime-registry-pulls" and .metadata.namespace == strenv(kp_runtime_pull_namespace)) | (.rules | length) == 2 and .rules[0].apiGroups == [""] and .rules[0].resources == ["secrets"] and .rules[0].verbs == ["get"] and (.rules[0].resourceNames | join(",")) == strenv(kp_runtime_pull_names) and .rules[1].apiGroups == [""] and .rules[1].resources == ["secrets"] and .rules[1].verbs == ["create"] and (.rules[1] | has("resourceNames") | not)' "${kp_tmp}/runtime-registry-pulls.yaml")" == "true" ]]
  [[ "$(kp_runtime_pull_namespace="${kp_runtime_pull_namespace}" yq eval-all 'select(.kind == "RoleBinding" and .metadata.labels."app.kubernetes.io/component" == "runtime-registry-pulls" and .metadata.namespace == strenv(kp_runtime_pull_namespace)) | .subjects == [{"kind":"ServiceAccount","name":"runtime-pulls-worker","namespace":"kuberploy-e2e-render"}] and .roleRef == {"apiGroup":"rbac.authorization.k8s.io","kind":"Role","name":"runtime-pulls-runtime-registry-pulls"}' "${kp_tmp}/runtime-registry-pulls.yaml")" == "true" ]]
  [[ "$(kp_runtime_pull_namespace="${kp_runtime_pull_namespace}" kp_runtime_pull_pair_a="${kp_runtime_pull_pair_a}" kp_runtime_pull_pair_b="${kp_runtime_pull_pair_b}" kp_runtime_pull_policy="${kp_runtime_pull_policy}" yq eval-all 'select(.kind == "ValidatingAdmissionPolicy" and .metadata.name == strenv(kp_runtime_pull_policy)) | .spec.failurePolicy == "Fail" and (.spec.validations | length) == 3 and ([.spec.validations[].expression] | join("\n")) as $expressions | ($expressions | contains("system:serviceaccount:kuberploy-e2e-render:runtime-pulls-worker")) and ($expressions | contains(strenv(kp_runtime_pull_namespace))) and ($expressions | contains(strenv(kp_runtime_pull_pair_a))) and ($expressions | contains(strenv(kp_runtime_pull_pair_b))) and ($expressions | contains("oldObject.metadata.name.startsWith('kuberploy-pull-')")) and ($expressions | contains("kubernetes.io/dockerconfigjson")) and ($expressions | contains("object.immutable == true")) and ($expressions | contains("object.stringData.size() == 0")) and ($expressions | contains("<= 87384"))' "${kp_tmp}/runtime-registry-pulls.yaml")" == "true" ]]
  [[ "$(kp_runtime_pull_namespace="${kp_runtime_pull_namespace}" kp_runtime_pull_policy="${kp_runtime_pull_policy}" yq eval-all 'select(.kind == "ValidatingAdmissionPolicyBinding" and .metadata.name == strenv(kp_runtime_pull_policy)) | .spec.policyName == strenv(kp_runtime_pull_policy) and .spec.validationActions == ["Deny"] and .spec.matchResources.namespaceSelector == {"matchLabels":{"kubernetes.io/metadata.name":strenv(kp_runtime_pull_namespace)}}' "${kp_tmp}/runtime-registry-pulls.yaml")" == "true" ]]
done
...
[[ "$(yq '.config.edgeRuntime.profiles.traefik.sslip == {"mode":"auto-first-ip"}' "${kp_tmp}/edge-runtime-values.yaml")" == "true" ]]
...
[[ "$(yq eval-all 'select(.kind == "ClusterRole" and .metadata.labels."app.kubernetes.io/component" == "edge-runtime") | (.rules | length) == 4 and .rules[0].apiGroups == ["networking.k8s.io"] and .rules[0].resources == ["ingressclasses"] and .rules[0].resourceNames == ["traefik"] and (.rules[1].resourceNames | length) == 10 and (.rules[2].resourceNames | length) == 6 and .rules[3].apiGroups == ["cert-manager.io"] and .rules[3].resources == ["clusterissuers"] and (.rules[3].resourceNames | join(",")) == "kuberploy-letsencrypt-production,kuberploy-letsencrypt-staging"' "${kp_tmp}/edge-runtime.yaml")" == "true" ]]
...
for kp_edge_mutation in \
  ...
  '.config.edgeRuntime.profiles.traefik.deployment.image = "docker.io/library/traefik:v3.7.10"' \
  ...
  '.config.edgeRuntime.profiles.externalDNS[0].deployment.image = "registry.k8s.io/external-dns/external-dns:v0.21.0"' \
  '.networkPolicy.enabled = false' \
  '.networkPolicy.kubeAPIServerCIDRs = []' \
  '.components.api.image.reference = "kuberploy-api:mutable"' \
  '.components.worker.image.reference = "kuberploy-worker:mutable"' \
...
[[ "$(yq eval-all '[select(.kind == "NetworkPolicy" and .metadata.labels."app.kubernetes.io/component" == "api") | .spec.egress[] | select(.to[0].ipBlock.cidr == "10.43.0.1/32")] | length' "${kp_tmp}/certificate-issuer-observer.yaml" | tail -1)" == "0" ]]
...
for kp_runtime_namespace in apps-production apps-staging; do
  [[ "$(yq eval-all 'select(.kind == "Role" and .metadata.namespace == strenv(kp_runtime_namespace) and (.metadata.name | test("-runtime-secrets-api$"))) | .rules == [{"apiGroups":["bitnami.com"],"resources":["sealedsecrets"],"verbs":["get","create","delete"]}]' "${kp_tmp}/runtime-secrets.yaml")" == "true" ]]
  [[ "$(yq eval-all 'select(.kind == "Role" and .metadata.namespace == strenv(kp_runtime_namespace) and (.metadata.name | test("-runtime-secrets-worker$"))) | .rules == [{"apiGroups":["bitnami.com"],"resources":["sealedsecrets"],"verbs":["get"]}]' "${kp_tmp}/runtime-secrets.yaml")" == "true" ]]
done
...
yq '.upgrade.migrationJob.enabled = true | .upgrade.migrationJob.command = ["/kuberploy-api", "migrate"]' \
  "${kp_root}/test/e2e/fixtures/platform-values.yaml" > "${kp_tmp}/upgrade-job.yaml"
helm template upgrade-seam "${kp_root}/charts/kuberploy" -f "${kp_tmp}/upgrade-job.yaml" > "${kp_tmp}/upgrade-job-render.yaml"
...
[[ "$(yq eval-all '[select(.kind == "NetworkPolicy" and .metadata.name == "upgrade-seam-migration") | .spec.egress[] | select(.to[0].namespaceSelector.matchLabels."kubernetes.io/metadata.name" == "kuberploy-postgresql" and .to[0].podSelector.matchLabels."app.kubernetes.io/name" == "kuberploy-postgresql" and (.ports | length) == 1 and .ports[0].port == 5432)] | length' "${kp_tmp}/upgrade-job-render.yaml" | tail -1)" == "1" ]]
...
[[ "$(yq eval-all '[select(.kind == "NetworkPolicy" and .metadata.name == "upgrade-seam-migration") | .spec.egress[] | .ports[].port | select(. != 5432)] | length' "${kp_tmp}/upgrade-job-render.yaml" | tail -1)" == "0" ]]
...
helm template local-git "${kp_root}/deploy/orbstack/local-git" \
...
if rg -n ":latest([@[:space:]\"']|$)" \
  ...
  "${kp_root}/deploy/orbstack" "${kp_tmp}"; then
...
for kp_template in appproject root-application; do
  sed -e 's|@@RUN_ID@@|render|g' \
      ...
      "${kp_root}/deploy/orbstack/argo-bootstrap/${kp_template}.yaml.tmpl" | yq eval '.' >/dev/null
done
sed -e 's|@@RUN_ID@@|render|g' \
    -e 's|@@RUN_NAMESPACE@@|kuberploy-e2e-render|g' \
    -e 's|@@GIT_URL@@|git://kuberploy-git.kuberploy-e2e-render.svc.cluster.local/kuberploy-environments.git|g' \
