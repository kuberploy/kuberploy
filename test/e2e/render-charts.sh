...
yq '.spec.routes[0].tls = {"mode":"letsencrypt","issuerRef":"letsencrypt-production","redirectHttp":true}' \
  "${kp_root}/test/e2e/fixtures/appconfig-backend.yaml" > "${kp_tmp}/runtime-letsencrypt-values.yaml"
helm template runtime-letsencrypt "${kp_root}/charts/kuberploy-runtime" \
  --namespace kuberploy-e2e-render \
...
# A tenant's advanced deployment override must not be able to add a second
# container, an initContainer, or a hostPath volume -- only containers[0]'s
# non-identity fields are meant to be adjustable.
if helm template invalid-override "${kp_root}/charts/kuberploy-runtime" \
  -f "${kp_root}/charts/kuberploy-runtime/testdata/resource-overrides.yaml" \
  --set-json 'spec.overrides.deployment.spec.template.spec.containers=[{"name":"app"},{"name":"sidecar","image":"evil:latest","securityContext":{"runAsUser":0}}]' >/dev/null 2>&1; then
  printf 'runtime chart accepted a second override container\n' >&2
  exit 1
fi
if helm template invalid-override "${kp_root}/charts/kuberploy-runtime" \
  -f "${kp_root}/charts/kuberploy-runtime/testdata/resource-overrides.yaml" \
  --set-json 'spec.overrides.deployment.spec.template.spec.initContainers=[{"name":"evil","image":"evil:latest"}]' >/dev/null 2>&1; then
  printf 'runtime chart accepted an override initContainer\n' >&2
  exit 1
fi
if helm template invalid-override "${kp_root}/charts/kuberploy-runtime" \
  -f "${kp_root}/charts/kuberploy-runtime/testdata/resource-overrides.yaml" \
  --set-json 'spec.overrides.deployment.spec.template.spec.volumes=[{"name":"evil","hostPath":{"path":"/"}}]' >/dev/null 2>&1; then
  printf 'runtime chart accepted an override hostPath volume\n' >&2
  exit 1
fi
...
[[ "$(yq eval-all 'select(.kind == "Ingress" and .metadata.name == "kuberploy") | [.metadata.annotations."cert-manager.io/private-key-algorithm",.metadata.annotations."cert-manager.io/private-key-size"] | join(",")' "${kp_tmp}/platform-tls-redirect.yaml")" == "ECDSA,256" ]]
...
[[ "$(yq eval-all 'select(.kind == "Deployment") | .spec.template.spec.securityContext.runAsUser' "${kp_tmp}/runtime.yaml")" == "null" ]]
[[ "$(yq eval-all 'select(.kind == "Deployment") | .spec.template.spec.containers[0].securityContext.readOnlyRootFilesystem' "${kp_tmp}/runtime.yaml")" == "null" ]]
[[ "$(yq eval-all 'select(.kind == "Deployment") | .spec.template.spec.containers[0].securityContext.allowPrivilegeEscalation' "${kp_tmp}/runtime.yaml")" == "false" ]]
...
[[ "$(yq eval-all 'select(.kind == "Deployment") | .spec.template.spec.imagePullSecrets[0].name' "${kp_tmp}/private-pull-render.yaml")" == "kuberploy-pull-c9e287443a826194b652e4c9" ]]
...
[[ "$(yq eval-all '[select(.kind == "ValidatingAdmissionPolicy" or .kind == "ValidatingAdmissionPolicyBinding") | select(.metadata.labels."app.kubernetes.io/component" == "argo-appset-refresh-admission")] | length' "${kp_tmp}/platform.yaml" | tail -1)" == "0" ]]
...
yq '.config.githubApp.enabled = true |
    ...
    .builder.builderAgentImage = "ghcr.io/kuberploy/kuberploy-builder-agent:0.1.0-rc.487" |
...
yq '.config.gitProjection.enabled = true | .config.gitProjection.chartVersion = "0.1.0-rc.487"' \
...
[[ "$(yq eval-all 'select(.kind == "ConfigMap") | .data.KUBERPLOY_GIT_PROJECTION_CHART_VERSION' "${kp_tmp}/git-projection.yaml")" == "0.1.0-rc.487" ]]
...
[[ "$(yq eval-all -o=json -I=0 'select(.kind == "Role" and .metadata.labels."app.kubernetes.io/component" == "argo-desired-state") | .rules' "${kp_tmp}/argo-desired-state.yaml")" == '[{"apiGroups":["argoproj.io"],"resources":["applications"],"resourceNames":["kuberploy-platform-root"],"verbs":["get","patch"]},{"apiGroups":["argoproj.io"],"resources":["applicationsets"],"verbs":["get","patch"]},{"apiGroups":[""],"resources":["secrets"],"verbs":["create","patch","delete"]}]' ]]
...
kp_argo_appset_refresh_admission_name='argo-desired-argo-appset-refresh-ff423e40c9'
export kp_argo_appset_refresh_admission_name
[[ "$(yq eval-all -o=json 'select(.kind == "ValidatingAdmissionPolicy" and .metadata.name == strenv(kp_argo_appset_refresh_admission_name))' "${kp_tmp}/argo-desired-state.yaml" | jq '.spec.failurePolicy == "Fail" and .spec.matchConstraints.matchPolicy == "Exact" and (.spec.matchConditions | length) == 2 and (.spec.validations | length) == 2')" == "true" ]]
[[ "$(yq eval-all -o=json -I=0 'select(.kind == "ValidatingAdmissionPolicy" and .metadata.name == strenv(kp_argo_appset_refresh_admission_name)) | .spec.matchConstraints.resourceRules' "${kp_tmp}/argo-desired-state.yaml")" == '[{"apiGroups":["argoproj.io"],"apiVersions":["v1alpha1"],"operations":["UPDATE"],"resources":["applicationsets"],"scope":"Namespaced"}]' ]]
[[ "$(yq eval-all -o=json 'select(.kind == "ValidatingAdmissionPolicy" and .metadata.name == strenv(kp_argo_appset_refresh_admission_name))' "${kp_tmp}/argo-desired-state.yaml" | jq '[.spec.validations[].expression] | join("\n") | contains("request.name == '\''kp-e-'\'' + object.metadata.labels['\''kuberploy.io/environment-id'\''].replace('\''-'\'', '\'''\'')") and contains("object.spec == oldObject.spec") and contains("argocd.argoproj.io/application-set-refresh") and contains("== '\''true'\''")')" == "true" ]]
[[ "$(yq eval-all -o=json 'select(.kind == "ValidatingAdmissionPolicyBinding" and .metadata.name == strenv(kp_argo_appset_refresh_admission_name))' "${kp_tmp}/argo-desired-state.yaml" | jq --arg name "${kp_argo_appset_refresh_admission_name}" '.spec.policyName == $name and .spec.validationActions == ["Deny"] and .spec.matchResources.namespaceSelector == {"matchLabels":{"kubernetes.io/metadata.name":"kuberploy-e2e-render"}}')" == "true" ]]
...
[[ "$(yq eval-all '[select(.kind == "Deployment" and (.metadata.labels."app.kubernetes.io/component" == "api" or .metadata.labels."app.kubernetes.io/component" == "worker")) | .spec.template.spec.containers[0].env[] | select(.name | test("^KUBERPLOY_RUNTIME_REGISTRY_PULL"))] | length' "${kp_tmp}/runtime-registry-pulls.yaml" | tail -1)" == "20" ]]
...
for kp_runtime_pull_namespace in apps-production apps-staging; do
  case "${kp_runtime_pull_namespace}" in
    apps-production)
      kp_runtime_pull_name_a="kuberploy-pull-a14e8553d5ded5b74722fd65"
      kp_runtime_pull_name_b="kuberploy-pull-37e9f37a0d53c1298691d109"
...
yq '.config.runtimeRegistryPulls.namespaces = [] | .config.runtimeRegistryPulls.namespacePrefixes = ["kp-"]' \
  "${kp_tmp}/runtime-registry-pulls-values.yaml" > "${kp_tmp}/runtime-registry-pulls-managed-values.yaml"
helm template runtime-pulls "${kp_root}/charts/kuberploy" --namespace kuberploy-e2e-render \
  -f "${kp_tmp}/runtime-registry-pulls-managed-values.yaml" > "${kp_tmp}/runtime-registry-pulls-managed.yaml"
[[ "$(yq eval-all 'select(.kind == "ConfigMap") | .data.KUBERPLOY_RUNTIME_REGISTRY_PULL_NAMESPACE_PREFIXES' "${kp_tmp}/runtime-registry-pulls-managed.yaml")" == "kp-" ]]
[[ "$(yq eval-all -o=json 'select(.kind == "ClusterRole" and .metadata.name == "runtime-pulls-runtime-registry-pulls-managed")' "${kp_tmp}/runtime-registry-pulls-managed.yaml" | jq '(.rules | length)==2 and .rules[0].verbs==["get"] and .rules[0].resourceNames==["kuberploy-pull-a14e8553d5ded5b74722fd65","kuberploy-pull-37e9f37a0d53c1298691d109"] and .rules[1].verbs==["create"] and (.rules[1]|has("resourceNames")|not)')" == "true" ]]
[[ "$(yq eval-all -o=json 'select(.kind == "ValidatingAdmissionPolicy" and .metadata.name == "runtime-pulls-registry-pulls-managed")' "${kp_tmp}/runtime-registry-pulls-managed.yaml" | jq '([.spec.validations[].expression] | join("\n")) as $expressions | (.spec.validations | length)==4 and ($expressions|contains("request.userInfo.username != '\''system:serviceaccount:kuberploy-e2e-render:runtime-pulls-worker'\''")) and ($expressions|contains("request.operation == '\''DELETE'\'' && has(namespaceObject.metadata.deletionTimestamp)"))')" == "true" ]]
...
[[ "$(yq eval-all '[select(.kind == "ClusterRole" or .kind == "ClusterRoleBinding") | select(.metadata.name | test("runtime-secrets-(api|worker)-managed$"))] | length' "${kp_tmp}/runtime-secrets.yaml" | tail -1)" == "4" ]]
[[ "$(yq eval-all '[select(.kind == "ClusterRole" and (.metadata.name | test("runtime-secrets-api-managed$"))) | .rules[] | select((.apiGroups | join(",")) == "bitnami.com" and (.resources | join(",")) == "sealedsecrets" and (.verbs | join(",")) == "get,create,delete")] | length' "${kp_tmp}/runtime-secrets.yaml" | tail -1)" == "1" ]]
[[ "$(yq eval-all '[select(.kind == "ClusterRole" and (.metadata.name | test("runtime-secrets-worker-managed$"))) | .rules[] | select((.apiGroups | join(",")) == "bitnami.com" and (.resources | join(",")) == "sealedsecrets" and (.verbs | join(",")) == "get")] | length' "${kp_tmp}/runtime-secrets.yaml" | tail -1)" == "1" ]]
...
[[ "$(yq eval-all '[select(.kind == "ValidatingAdmissionPolicyBinding" and .metadata.labels."app.kubernetes.io/component" == "runtime-secrets-admission" and (.spec.validationActions | join(",")) == "Deny" and (.spec | has("matchResources") | not))] | length' "${kp_tmp}/runtime-secrets.yaml" | tail -1)" == "1" ]] || { printf 'runtime-secret admission binding is not globally enforcing managed namespace boundaries\n' >&2; exit 1; }
[[ "$(yq eval-all 'select(.kind == "ValidatingAdmissionPolicy" and .metadata.labels."app.kubernetes.io/component" == "runtime-secrets-admission") | ([.spec.validations[].expression] | join("\n")) as $expressions | (($expressions | contains("apps-production")) and ($expressions | contains("apps-staging")) and ($expressions | contains("request.namespace.startsWith")) and ($expressions | contains("kp-")) and ($expressions | contains("kuberploy.io/runtime-namespace")))' "${kp_tmp}/runtime-secrets.yaml")" == "true" ]] || { printf 'runtime-secret admission policy omitted exact and labeled prefix namespace fences\n' >&2; exit 1; }
...
[[ "$(yq eval-all 'select(.kind == "ValidatingAdmissionPolicy" and .metadata.labels."app.kubernetes.io/component" == "runtime-secrets-admission") | ([.spec.validations[].expression] | join("\n")) as $expressions | (($expressions | contains("kubernetes.io/dockerconfigjson")) and ($expressions | contains(".dockerconfigjson")) and ($expressions | contains("encryptedData.size() == 1")))' "${kp_tmp}/certificate-observation.yaml")" == "true" ]] || { printf 'runtime-secret admission policy did not retain the exact dockerconfigjson registry-credential SealedSecret envelope\n' >&2; exit 1; }
...
yq '.config.gitProjection.enabled = true | .config.gitProjection.chartVersion = "0.1.0-rc.487"' "${kp_root}/test/e2e/fixtures/platform-values.yaml" > "${kp_tmp}/git-projection-no-github.yaml"
...
[[ "$(yq eval-all 'select(.kind == "Job" and .metadata.labels."app.kubernetes.io/component" == "migration") | .spec.template.spec.containers[0].image' "${kp_tmp}/upgrade-job-render.yaml")" == "ghcr.io/kuberploy/kuberploy-migration:0.1.0-rc.487" ]]
...
[[ "$(yq eval-all 'select(.kind == "Deployment" and .metadata.labels."app.kubernetes.io/component" == "api") | .spec.template.spec.containers[0].startupProbe.failureThreshold' "${kp_tmp}/upgrade-job-render.yaml")" == "12" ]]
[[ "$(yq eval-all 'select(.kind == "Deployment" and .metadata.labels."app.kubernetes.io/component" == "worker") | .spec.template.spec.containers[0].startupProbe // "absent"' "${kp_tmp}/upgrade-job-render.yaml")" == "absent" ]]
...
[[ "$(yq eval-all 'select(.kind == "Job" and .metadata.name == "bootstrap-token-bootstrap-token") | .spec.activeDeadlineSeconds' "${kp_tmp}/bootstrap-token.yaml")" == "1800" ]]
