...
yq '.config.githubApp.enabled = true |
    ...
    .builder.builderAgentImage = "ghcr.io/kuberploy/kuberploy-builder-agent:0.1.0-rc.19" |
...
yq '.config.gitProjection.enabled = true | .config.gitProjection.chartVersion = "0.1.0-rc.19"' \
...
[[ "$(yq eval-all 'select(.kind == "ConfigMap") | .data.KUBERPLOY_GIT_PROJECTION_CHART_VERSION' "${kp_tmp}/git-projection.yaml")" == "0.1.0-rc.19" ]]
...
yq '.config.gitProjection.enabled = true | .config.gitProjection.chartVersion = "0.1.0-rc.19"' "${kp_root}/test/e2e/fixtures/platform-values.yaml" > "${kp_tmp}/git-projection-no-github.yaml"
