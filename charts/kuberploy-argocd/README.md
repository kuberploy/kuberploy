...
Managed mode uses the exact multi-platform Argo CD image digest, retained CRDs,
restricted containers, required resource bounds, default-deny ingress and
egress, ClusterIP-only server access, no local admin, no default direct role,
...
The repository/API CIDRs are explicit operator inputs. P0 repository transport
is HTTPS-only; there is no ambient SSH or unrestricted-port egress. Direct Argo
