...
{{- if $github.enabled -}}
  ...
  {{- if or (not $rootBootstrap.enabled) (ne $rootBootstrap.bindingID $github.platformBindingID) -}}{{ fail "GitHub desired-state integration requires the exact platform root bootstrap binding" }}{{- end -}}
...
{{- if $runtimeSecrets.enabled -}}
  ...
  {{- if or (and (empty $runtimeSecrets.namespaces) (empty $runtimeSecrets.namespacePrefixes)) (ne (len $runtimeSecrets.namespaces) (len (uniq $runtimeSecrets.namespaces))) (ne (len $runtimeSecrets.namespacePrefixes) (len (uniq $runtimeSecrets.namespacePrefixes))) (not (regexMatch "^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$" $runtimeSecrets.fingerprintKeyID)) (not (regexMatch "^[a-z0-9](?:[-a-z0-9]{0,61}[a-z0-9])?$" $runtimeSecrets.fingerprintSecretName)) (not (regexMatch "^[A-Za-z0-9._-]{1,253}$" $runtimeSecrets.fingerprintSecretKey)) -}}{{ fail "runtime secret identities and namespace policy are invalid" }}{{- end -}}
  {{- range $runtimeSecrets.namespaces -}}
    {{- if or (not (regexMatch "^[a-z0-9](?:[-a-z0-9]{0,61}[a-z0-9])?$" .)) (has . (list "argocd" "cert-manager" "default" "kube-node-lease" "kube-public" "kube-system" "kuberploy-build-dind" "kuberploy-monitoring" "kuberploy-system" "sealed-secrets")) -}}{{ fail "runtime secrets accept only exact non-platform Environment namespaces" }}{{- end -}}
  {{- end -}}
  {{- range $runtimeSecrets.namespacePrefixes -}}
    {{- if not (regexMatch "^[a-z0-9](?:[-a-z0-9]{0,60}[a-z0-9])?-$" .) -}}{{ fail "runtime secret namespace prefixes must be canonical managed Environment prefixes" }}{{- end -}}
  {{- end -}}
{{- end -}}
...
{{- if $registry.enabled -}}
  ...
  {{- end -}}
  {{- if or (not (has $registry.exposureMode (list "ingress" "loadBalancer"))) (not (regexMatch "^[a-z0-9](?:[-a-z0-9]{0,61}[a-z0-9])?$" $registry.authSecretName)) (not (regexMatch "^[a-z0-9](?:[-a-z0-9]{0,61}[a-z0-9])?$" $registry.helmCredentialSecretName)) (not (regexMatch "^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$" $registry.secretRevision)) (not (regexMatch "^(?:(?:[0-9]{1,3}\\.){3}[0-9]{1,3}|[a-z0-9](?:[-a-z0-9]{0,61}[a-z0-9])?(?:\\.[a-z0-9](?:[-a-z0-9]{0,61}[a-z0-9])?)+)$" $registry.endpoint)) (not (regexMatch "^[a-z0-9](?:[-a-z0-9]{0,61}[a-z0-9])?$" $registry.tlsSecretName)) -}}{{ fail "managed registry integration requires exact auth, Helm OCI repository, endpoint, and TLS identities" }}{{- end -}}
