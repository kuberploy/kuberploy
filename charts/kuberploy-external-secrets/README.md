...
and applies explicit default-deny NetworkPolicies. Only namespaced
...
Provider access is HTTPS-only through explicit operator-supplied CIDRs. For a
provider that needs another port, deploy a narrowly controlled HTTPS egress
proxy instead of broadening the controller pod. Provider credentials must be
delivered through the provider's workload identity or an existing Secret; they
must never be put in Helm values or Git.
