...
Set `ignoreNamespaceSelectors: true`, place each generated monitor beside its target and grant the Prometheus ServiceAccount discovery access through RoleBindings only in managed namespaces. Generated endpoints use `honorLabels: false`, while Prometheus enforces label conflict overrides plus target, sample, label-count and label-length limits. Admission policy rejects a cross-namespace or unrestricted target selector. NetworkPolicy permits `kuberploy-monitoring` only to each declared metrics port. Together these controls prevent a tenant monitor from scraping another namespace, a cloud metadata address or an arbitrary external target.
...
OrbStack supplies the local Docker/Buildx engine for fast development-image and
...
Commands select the OrbStack Docker context explicitly so an ambient engine is
...
Current implementation status (2026-08-09): the production code paths now
...
- [OrbStack Docker engine](https://docs.orbstack.dev/docker/)
