...
| Stopped App configuration copied by Environment clone | PostgreSQL until an explicit Start App publishes it to Git; the clone inherits repository/ref authority but creates no workload, Git write command, or provider mutation before that start |
...
7. A direct write advances after its verified commit. A protected write remains review-pending until the provider proves the pull request merge is visible on the exact target ref. Before that protected publication becomes terminal, Kuberploy refreshes the exact Environment ApplicationSet and wakes deployment observation. When the Environment and platform bindings share the same repository/ref, it first hard-refreshes the fixed root Application at the verified merge revision. Separate Environment repository/ref shards refresh only their existing platform-owned ApplicationSet; their commit must never be applied to the unrelated platform root. A failed required refresh leaves the publication merge-pending for retry. Argo sync and Deployment rollout health remain observed phases on the deployment-status resource.
...
- The Git provider sends verified push webhooks to the Kuberploy indexer; polling remains a repair path. After a provider-verified commit to the platform repository/ref, Kuberploy requests one admission-fenced metadata refresh of the installer-owned root Application so Argo re-reads that branch immediately. A protected Environment merge refreshes its exact existing ApplicationSet and wakes observation; it also refreshes the root first when both bindings share the same repository/ref. Separate Environment shards do not change or refresh an unrelated platform root. Protected publication stays merge-pending until its required refreshes succeed.
