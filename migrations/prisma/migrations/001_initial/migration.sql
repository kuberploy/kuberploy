...
CREATE TABLE public.secret_binding_versions (
    ...
    CONSTRAINT secret_binding_versions_check3 CHECK ((((state = 'staging'::text) AND (staged_at IS NULL)) OR ((state <> 'staging'::text) AND ((staged_at IS NOT NULL) OR (state = ANY (ARRAY['failed'::text, 'deleted'::text])))))),
