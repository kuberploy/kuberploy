...
--
-- Name: protect_helm_chart_approval(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.protect_helm_chart_approval() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
--
-- Name: protect_helm_chart_approval_documents(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.protect_helm_chart_approval_documents() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE FUNCTION public.validate_helm_chart_approval_document_insert() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
...
--
-- Name: validate_helm_protected_application_intent(); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: validate_helm_release_head(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.validate_helm_release_head() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
...
--
-- Name: validate_helm_release_revision(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.validate_helm_release_revision() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
...
--
-- Name: validate_helm_render_command(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.validate_helm_render_command() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
    durable_namespace text;
...
--
-- Name: validate_helm_render_result(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.validate_helm_render_result() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
    durable_input text;
...
CREATE TABLE public.helm_chart_approval_documents (
...
--
-- Name: helm_chart_approvals; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.helm_chart_approvals (
    ...
    CONSTRAINT helm_chart_approvals_oci_repository_check CHECK (((length(oci_repository) >= 12) AND (length(oci_repository) <= 512) AND (oci_repository = lower(oci_repository)) AND (oci_repository ~ '^oci://[a-z0-9][a-z0-9.-]*(?::[1-9][0-9]{0,4})?/[a-z0-9][a-z0-9._/-]*[a-z0-9]$'::text) AND (oci_repository !~ '[@?#[:space:]]'::text) AND (oci_repository !~ '(^|/)(\.|\.\.)($|/)'::text))),
    CONSTRAINT helm_chart_approvals_package_digest_check CHECK ((package_digest ~ '^sha256:[0-9a-f]{64}$'::text)),
    CONSTRAINT helm_chart_approvals_policy_version_check CHECK ((policy_version = 'external-helm-p0.v1'::text)),
...
--
-- Name: helm_protected_application_intents; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.helm_protected_application_intents (
    id uuid NOT NULL,
    release_revision_id uuid NOT NULL,
    payload_intent_id uuid NOT NULL,
    release_generation bigint NOT NULL,
    ...
    environment_generation bigint CONSTRAINT helm_protected_application_inte_environment_generation_not_null NOT NULL,
    catalog_digest text NOT NULL,
    planned_base_revision text CONSTRAINT helm_protected_application_inten_planned_base_revision_not_null NOT NULL,
    payload_revision text NOT NULL,
    ...
    commit_trailer text NOT NULL,
    publisher_contract text NOT NULL,
    publisher_config_digest text CONSTRAINT helm_protected_application_int_publisher_config_digest_not_null NOT NULL,
    message text NOT NULL,
    state text NOT NULL,
    next_attempt_at timestamp with time zone NOT NULL,
    attempts integer DEFAULT 0 NOT NULL,
    consecutive_failures integer DEFAULT 0 CONSTRAINT helm_protected_application_intent_consecutive_failures_not_null NOT NULL,
    last_failure_code text DEFAULT ''::text NOT NULL,
    lease_owner text,
    lease_epoch bigint DEFAULT 0 NOT NULL,
    lease_until timestamp with time zone,
    ...
    write_base_observed_at timestamp with time zone,
    committed_revision text DEFAULT ''::text NOT NULL,
    committed_parent_revision text DEFAULT ''::text CONSTRAINT helm_protected_application_i_committed_parent_revision_not_null NOT NULL,
    committed_at timestamp with time zone,
    verified_at timestamp with time zone,
    verified_path_digest text DEFAULT ''::text CONSTRAINT helm_protected_application_intent_verified_path_digest_not_null NOT NULL,
    provider_request text DEFAULT ''::text NOT NULL,
    created_at timestamp with time zone NOT NULL,
    updated_at timestamp with time zone NOT NULL,
    completed_at timestamp with time zone,
    ...
    CONSTRAINT helm_protected_application_intents_application_path_check CHECK (((length(application_path) >= 1) AND (length(application_path) <= 1024) AND (application_path !~ '(^/|/\.\.?(/|$)|//|\\|[[:cntrl:]])'::text))),
    CONSTRAINT helm_protected_application_intents_attempts_check CHECK (((attempts >= 0) AND (attempts <= 30))),
    CONSTRAINT helm_protected_application_intents_catalog_digest_check CHECK ((catalog_digest ~ '^sha256:[0-9a-f]{64}$'::text)),
    CONSTRAINT helm_protected_application_intents_check CHECK ((((precondition = 'create-if-absent'::text) AND (expected_etag = ''::text)) OR ((precondition = 'match-etag'::text) AND (expected_etag ~ '^"sha256:[0-9a-f]{64}"$'::text)))),
    CONSTRAINT helm_protected_application_intents_check1 CHECK ((commit_trailer = ('Kuberploy-Helm-Application-Intent: '::text || (id)::text))),
    CONSTRAINT helm_protected_application_intents_check2 CHECK (((updated_at >= created_at) AND (next_attempt_at >= created_at))),
    CONSTRAINT helm_protected_application_intents_check3 CHECK (((last_failure_code = ''::text) = (consecutive_failures = 0))),
    CONSTRAINT helm_protected_application_intents_check4 CHECK ((((write_base_revision = ''::text) AND (write_base_observed_at IS NULL)) OR ((write_base_revision <> ''::text) AND (write_base_observed_at IS NOT NULL) AND (write_base_observed_at >= created_at) AND (write_base_observed_at <= updated_at)))),
    CONSTRAINT helm_protected_application_intents_check5 CHECK ((((action = 'publish'::text) AND (operation = ANY (ARRAY['create'::text, 'update'::text])) AND (source_directory <> ''::text) AND (octet_length(content) > 0) AND (content_digest <> ''::text)) OR ((action = 'delete'::text) AND (operation = 'delete'::text) AND (source_directory = ''::text) AND (octet_length(content) = 0) AND (content_digest = ''::text)))),
    CONSTRAINT helm_protected_application_intents_check6 CHECK ((((operation = 'create'::text) AND (precondition = 'create-if-absent'::text)) OR ((operation = ANY (ARRAY['update'::text, 'delete'::text])) AND (precondition = 'match-etag'::text)))),
    CONSTRAINT helm_protected_application_intents_check7 CHECK ((((lease_owner IS NULL) AND (lease_until IS NULL)) OR ((lease_owner IS NOT NULL) AND (lease_until IS NOT NULL) AND (lease_epoch > 0) AND (lease_until > updated_at)))),
    CONSTRAINT helm_protected_application_intents_check8 CHECK ((((state = 'pending'::text) AND (lease_owner IS NULL) AND (committed_revision = ''::text) AND (committed_parent_revision = ''::text) AND (committed_at IS NULL) AND (verified_at IS NULL) AND (verified_path_digest = ''::text) AND (provider_request = ''::text) AND (completed_at IS NULL)) OR ((state = 'claimed'::text) AND (lease_owner IS NOT NULL) AND (attempts > 0) AND (committed_revision = ''::text) AND (committed_parent_revision = ''::text) AND (committed_at IS NULL) AND (verified_at IS NULL) AND (verified_path_digest = ''::text) AND (provider_request = ''::text) AND (completed_at IS NULL)) OR ((state = 'git-committed'::text) AND (lease_owner IS NOT NULL) AND (write_base_revision <> ''::text) AND (committed_revision <> ''::text) AND (committed_parent_revision = write_base_revision) AND (committed_at IS NOT NULL) AND (verified_at IS NULL) AND (verified_path_digest = ''::text) AND (provider_request = ''::text) AND (completed_at IS NULL)) OR ((state = 'verified'::text) AND (lease_owner IS NULL) AND (write_base_revision <> ''::text) AND (committed_revision <> ''::text) AND (committed_parent_revision = write_base_revision) AND (committed_at IS NOT NULL) AND (verified_at IS NOT NULL) AND (verified_at >= committed_at) AND (verified_path_digest = content_digest) AND (provider_request <> ''::text) AND (completed_at = verified_at)) OR ((state = 'failed'::text) AND (lease_owner IS NULL) AND (verified_at IS NULL) AND (verified_path_digest = ''::text) AND (provider_request = ''::text) AND (completed_at IS NOT NULL) AND (completed_at >= created_at) AND (committed_revision = ''::text) AND (committed_parent_revision = ''::text) AND (committed_at IS NULL)) OR ((state = 'superseded'::text) AND (lease_owner IS NULL) AND (committed_revision = ''::text) AND (committed_parent_revision = ''::text) AND (committed_at IS NULL) AND (verified_at IS NULL) AND (verified_path_digest = ''::text) AND (provider_request = ''::text) AND (completed_at IS NOT NULL) AND (completed_at >= created_at)))),
    CONSTRAINT helm_protected_application_intents_committed_revision_check CHECK (((committed_revision = ''::text) OR (committed_revision ~ '^(?:[0-9a-f]{40}|[0-9a-f]{64})$'::text))),
    CONSTRAINT helm_protected_application_intents_consecutive_failures_check CHECK (((consecutive_failures >= 0) AND (consecutive_failures <= 30))),
    CONSTRAINT helm_protected_application_intents_content_check CHECK (((octet_length(content) >= 0) AND (octet_length(content) <= 32768))),
    CONSTRAINT helm_protected_application_intents_content_digest_check CHECK (((content_digest = ''::text) OR (content_digest ~ '^sha256:[0-9a-f]{64}$'::text))),
    CONSTRAINT helm_protected_application_intents_environment_generation_check CHECK ((environment_generation > 0)),
    CONSTRAINT helm_protected_application_intents_environment_revision_check CHECK ((environment_revision ~ '^(?:[0-9a-f]{40}|[0-9a-f]{64})$'::text)),
    CONSTRAINT helm_protected_application_intents_environment_target_ref_check CHECK (((length(environment_target_ref) >= 1) AND (length(environment_target_ref) <= 255) AND (environment_target_ref !~ '[[:cntrl:]]'::text))),
    CONSTRAINT helm_protected_application_intents_intent_digest_check CHECK ((intent_digest ~ '^sha256:[0-9a-f]{64}$'::text)),
    CONSTRAINT helm_protected_application_intents_last_failure_code_check CHECK (((last_failure_code = ''::text) OR (last_failure_code ~ '^[a-z][a-z0-9.-]{0,62}$'::text))),
    CONSTRAINT helm_protected_application_intents_lease_epoch_check CHECK ((lease_epoch >= 0)),
    CONSTRAINT helm_protected_application_intents_lease_owner_check CHECK (((lease_owner IS NULL) OR ((length(lease_owner) >= 16) AND (length(lease_owner) <= 128) AND (lease_owner ~ '^[A-Za-z0-9][A-Za-z0-9._:-]{15,127}$'::text)))),
    CONSTRAINT helm_protected_application_intents_message_check CHECK (((length(message) >= 1) AND (length(message) <= 512) AND (message !~ '[[:cntrl:]]'::text))),
    CONSTRAINT helm_protected_application_intents_operation_check CHECK ((operation = ANY (ARRAY['create'::text, 'update'::text, 'delete'::text]))),
    CONSTRAINT helm_protected_application_intents_payload_path_check CHECK (((length(payload_path) >= 1) AND (length(payload_path) <= 1024) AND (payload_path !~ '(^/|/\.\.?(/|$)|//|\\|[[:cntrl:]])'::text))),
    CONSTRAINT helm_protected_application_intents_payload_revision_check CHECK ((payload_revision ~ '^(?:[0-9a-f]{40}|[0-9a-f]{64})$'::text)),
    CONSTRAINT helm_protected_application_intents_planned_base_revision_check CHECK ((planned_base_revision ~ '^(?:[0-9a-f]{40}|[0-9a-f]{64})$'::text)),
    CONSTRAINT helm_protected_application_intents_platform_target_ref_check CHECK (((length(platform_target_ref) >= 1) AND (length(platform_target_ref) <= 255) AND (platform_target_ref !~ '[[:cntrl:]]'::text))),
    CONSTRAINT helm_protected_application_intents_precondition_check CHECK ((precondition = ANY (ARRAY['create-if-absent'::text, 'match-etag'::text]))),
    CONSTRAINT helm_protected_application_intents_provider_request_check CHECK (((length(provider_request) <= 256) AND (provider_request !~ '[[:cntrl:]]'::text))),
    CONSTRAINT helm_protected_application_intents_publisher_contract_check CHECK ((publisher_contract = 'helm-protected-publisher.v1'::text)),
    CONSTRAINT helm_protected_application_intents_release_generation_check CHECK ((release_generation > 0)),
...
--
-- Name: helm_protected_payload_intents; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.helm_protected_payload_intents (
    id uuid NOT NULL,
    release_revision_id uuid NOT NULL,
    release_generation bigint NOT NULL,
    project_id uuid NOT NULL,
    ...
    content bytea NOT NULL,
    content_digest text NOT NULL,
    manifest_inventory_digest text,
    manifest_resource_count integer,
    intent_digest text NOT NULL,
    commit_trailer text NOT NULL,
    publisher_contract text NOT NULL,
    publisher_config_digest text NOT NULL,
    ...
    write_base_observed_at timestamp with time zone,
    committed_revision text DEFAULT ''::text NOT NULL,
    committed_parent_revision text DEFAULT ''::text CONSTRAINT helm_protected_payload_inten_committed_parent_revision_not_null NOT NULL,
    committed_at timestamp with time zone,
    verified_at timestamp with time zone,
    verified_path_digest text DEFAULT ''::text NOT NULL,
    provider_request text DEFAULT ''::text NOT NULL,
    created_at timestamp with time zone NOT NULL,
    updated_at timestamp with time zone NOT NULL,
    completed_at timestamp with time zone,
    ...
    CONSTRAINT helm_protected_payload_intents_check CHECK ((commit_trailer = ('Kuberploy-Helm-Payload-Intent: '::text || (id)::text))),
    CONSTRAINT helm_protected_payload_intents_check1 CHECK (((updated_at >= created_at) AND (next_attempt_at >= created_at))),
    CONSTRAINT helm_protected_payload_intents_check2 CHECK (((last_failure_code = ''::text) = (consecutive_failures = 0))),
    CONSTRAINT helm_protected_payload_intents_check3 CHECK ((((write_base_revision = ''::text) AND (write_base_observed_at IS NULL)) OR ((write_base_revision <> ''::text) AND (write_base_observed_at IS NOT NULL) AND (write_base_observed_at >= created_at) AND (write_base_observed_at <= updated_at)))),
    CONSTRAINT helm_protected_payload_intents_check4 CHECK ((((action = 'publish'::text) AND (manifest_inventory_digest IS NOT NULL) AND (manifest_resource_count IS NOT NULL)) OR ((action = 'disable-receipt'::text) AND (manifest_inventory_digest IS NULL) AND (manifest_resource_count IS NULL)))),
    CONSTRAINT helm_protected_payload_intents_check5 CHECK ((((lease_owner IS NULL) AND (lease_until IS NULL)) OR ((lease_owner IS NOT NULL) AND (lease_until IS NOT NULL) AND (lease_epoch > 0) AND (lease_until > updated_at)))),
    CONSTRAINT helm_protected_payload_intents_check6 CHECK ((((state = 'pending'::text) AND (lease_owner IS NULL) AND (committed_revision = ''::text) AND (committed_parent_revision = ''::text) AND (committed_at IS NULL) AND (verified_at IS NULL) AND (verified_path_digest = ''::text) AND (provider_request = ''::text) AND (completed_at IS NULL)) OR ((state = 'claimed'::text) AND (lease_owner IS NOT NULL) AND (attempts > 0) AND (committed_revision = ''::text) AND (committed_parent_revision = ''::text) AND (committed_at IS NULL) AND (verified_at IS NULL) AND (verified_path_digest = ''::text) AND (provider_request = ''::text) AND (completed_at IS NULL)) OR ((state = 'git-committed'::text) AND (lease_owner IS NOT NULL) AND (write_base_revision <> ''::text) AND (committed_revision <> ''::text) AND (committed_parent_revision = write_base_revision) AND (committed_at IS NOT NULL) AND (verified_at IS NULL) AND (verified_path_digest = ''::text) AND (provider_request = ''::text) AND (completed_at IS NULL)) OR ((state = 'verified'::text) AND (lease_owner IS NULL) AND (write_base_revision <> ''::text) AND (committed_revision <> ''::text) AND (committed_parent_revision = write_base_revision) AND (committed_at IS NOT NULL) AND (verified_at IS NOT NULL) AND (verified_at >= committed_at) AND (verified_path_digest = content_digest) AND (provider_request <> ''::text) AND (completed_at = verified_at)) OR ((state = 'failed'::text) AND (lease_owner IS NULL) AND (verified_at IS NULL) AND (verified_path_digest = ''::text) AND (provider_request = ''::text) AND (completed_at IS NOT NULL) AND (completed_at >= created_at) AND (committed_revision = ''::text) AND (committed_parent_revision = ''::text) AND (committed_at IS NULL)) OR ((state = 'superseded'::text) AND (lease_owner IS NULL) AND (committed_revision = ''::text) AND (committed_parent_revision = ''::text) AND (committed_at IS NULL) AND (verified_at IS NULL) AND (verified_path_digest = ''::text) AND (provider_request = ''::text) AND (completed_at IS NOT NULL) AND (completed_at >= created_at)))),
    CONSTRAINT helm_protected_payload_intents_committed_parent_revision_check CHECK (((committed_parent_revision = ''::text) OR (committed_parent_revision ~ '^(?:[0-9a-f]{40}|[0-9a-f]{64})$'::text))),
    CONSTRAINT helm_protected_payload_intents_committed_revision_check CHECK (((committed_revision = ''::text) OR (committed_revision ~ '^(?:[0-9a-f]{40}|[0-9a-f]{64})$'::text))),
    CONSTRAINT helm_protected_payload_intents_consecutive_failures_check CHECK (((consecutive_failures >= 0) AND (consecutive_failures <= 30))),
    CONSTRAINT helm_protected_payload_intents_content_check CHECK (((octet_length(content) >= 1) AND (octet_length(content) <= 2097152))),
    CONSTRAINT helm_protected_payload_intents_content_digest_check CHECK ((content_digest ~ '^sha256:[0-9a-f]{64}$'::text)),
    CONSTRAINT helm_protected_payload_intents_environment_generation_check CHECK ((environment_generation > 0)),
    CONSTRAINT helm_protected_payload_intents_environment_revision_check CHECK ((environment_revision ~ '^(?:[0-9a-f]{40}|[0-9a-f]{64})$'::text)),
    CONSTRAINT helm_protected_payload_intents_environment_target_ref_check CHECK (((length(environment_target_ref) >= 1) AND (length(environment_target_ref) <= 255) AND (environment_target_ref !~ '[[:cntrl:]]'::text))),
    CONSTRAINT helm_protected_payload_intents_expected_etag_check CHECK ((expected_etag = ''::text)),
    CONSTRAINT helm_protected_payload_intents_intent_digest_check CHECK ((intent_digest ~ '^sha256:[0-9a-f]{64}$'::text)),
    CONSTRAINT helm_protected_payload_intents_last_failure_code_check CHECK (((last_failure_code = ''::text) OR (last_failure_code ~ '^[a-z][a-z0-9.-]{0,62}$'::text))),
    CONSTRAINT helm_protected_payload_intents_lease_epoch_check CHECK ((lease_epoch >= 0)),
    CONSTRAINT helm_protected_payload_intents_lease_owner_check CHECK (((lease_owner IS NULL) OR ((length(lease_owner) >= 16) AND (length(lease_owner) <= 128) AND (lease_owner ~ '^[A-Za-z0-9][A-Za-z0-9._:-]{15,127}$'::text)))),
    CONSTRAINT helm_protected_payload_intents_manifest_inventory_digest_check CHECK (((manifest_inventory_digest IS NULL) OR (manifest_inventory_digest ~ '^sha256:[0-9a-f]{64}$'::text))),
    CONSTRAINT helm_protected_payload_intents_manifest_resource_count_check CHECK (((manifest_resource_count IS NULL) OR ((manifest_resource_count >= 1) AND (manifest_resource_count <= 128)))),
    CONSTRAINT helm_protected_payload_intents_message_check CHECK (((length(message) >= 1) AND (length(message) <= 512) AND (message !~ '[[:cntrl:]]'::text))),
    CONSTRAINT helm_protected_payload_intents_path_check CHECK (((length(path) >= 1) AND (length(path) <= 1024) AND (path !~ '(^/|/\.\.?(/|$)|//|\\|[[:cntrl:]])'::text))),
    CONSTRAINT helm_protected_payload_intents_planned_base_revision_check CHECK ((planned_base_revision ~ '^(?:[0-9a-f]{40}|[0-9a-f]{64})$'::text)),
    CONSTRAINT helm_protected_payload_intents_platform_target_ref_check CHECK (((length(platform_target_ref) >= 1) AND (length(platform_target_ref) <= 255) AND (platform_target_ref !~ '[[:cntrl:]]'::text))),
    CONSTRAINT helm_protected_payload_intents_precondition_check CHECK ((precondition = 'create-if-absent'::text)),
    CONSTRAINT helm_protected_payload_intents_provider_request_check CHECK (((length(provider_request) <= 256) AND (provider_request !~ '[[:cntrl:]]'::text))),
    CONSTRAINT helm_protected_payload_intents_publisher_config_digest_check CHECK ((publisher_config_digest ~ '^sha256:[0-9a-f]{64}$'::text)),
    CONSTRAINT helm_protected_payload_intents_publisher_contract_check CHECK ((publisher_contract = 'helm-protected-publisher.v1'::text)),
...
--
-- Name: helm_release_heads; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.helm_release_heads (
    project_id uuid NOT NULL,
    environment_id uuid NOT NULL,
    application_id uuid NOT NULL,
    ...
    generation bigint NOT NULL,
    updated_at timestamp with time zone NOT NULL,
    CONSTRAINT helm_release_heads_generation_check CHECK ((generation > 0))
);


--
-- Name: helm_release_revisions; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.helm_release_revisions (
    id uuid NOT NULL,
    generation bigint NOT NULL,
    project_id uuid NOT NULL,
    environment_id uuid NOT NULL,
    ...
    request_id text NOT NULL,
    created_at timestamp with time zone NOT NULL,
    CONSTRAINT helm_release_revisions_action_check CHECK ((action = ANY (ARRAY['initial'::text, 'update'::text, 'retry'::text, 'disable'::text, 'rollback'::text]))),
    CONSTRAINT helm_release_revisions_check CHECK ((desired_enabled = (action <> 'disable'::text))),
    CONSTRAINT helm_release_revisions_check1 CHECK (((render_command_id IS NOT NULL) = desired_enabled)),
    CONSTRAINT helm_release_revisions_check2 CHECK ((((action = 'initial'::text) AND (generation = 1) AND (parent_revision_id IS NULL) AND (rollback_source_revision_id IS NULL) AND (base_intent_id IS NULL)) OR ((action = ANY (ARRAY['update'::text, 'retry'::text])) AND (generation > 1) AND (parent_revision_id IS NOT NULL) AND (rollback_source_revision_id IS NULL)) OR ((action = 'disable'::text) AND (generation > 1) AND (parent_revision_id IS NOT NULL) AND (rollback_source_revision_id IS NULL) AND (base_intent_id IS NOT NULL)) OR ((action = 'rollback'::text) AND (generation > 1) AND (parent_revision_id IS NOT NULL) AND (rollback_source_revision_id IS NOT NULL)))),
    CONSTRAINT helm_release_revisions_check3 CHECK (((rollback_source_revision_id IS NULL) OR (rollback_source_revision_id <> parent_revision_id))),
    CONSTRAINT helm_release_revisions_generation_check CHECK ((generation > 0)),
    CONSTRAINT helm_release_revisions_idempotency_key_check CHECK (((length(idempotency_key) >= 16) AND (length(idempotency_key) <= 128) AND (idempotency_key ~ '^[A-Za-z0-9][A-Za-z0-9._:-]{15,127}$'::text))),
    CONSTRAINT helm_release_revisions_intent_digest_check CHECK ((intent_digest ~ '^sha256:[0-9a-f]{64}$'::text)),
    CONSTRAINT helm_release_revisions_release_name_check CHECK (((length(release_name) >= 1) AND (length(release_name) <= 63) AND (release_name ~ '^[a-z0-9](?:[-a-z0-9]{0,61}[a-z0-9])?$'::text))),
    CONSTRAINT helm_release_revisions_request_id_check CHECK (((length(request_id) >= 1) AND (length(request_id) <= 128) AND (request_id !~ '[[:cntrl:]]'::text))),
    CONSTRAINT helm_release_revisions_values_digest_check CHECK ((values_digest ~ '^sha256:[0-9a-f]{64}$'::text)),
    CONSTRAINT helm_release_revisions_values_yaml_check CHECK (((octet_length(values_yaml) >= 1) AND (octet_length(values_yaml) <= 262144)))
);


--
-- Name: helm_render_commands; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.helm_render_commands (
    id uuid NOT NULL,
    idempotency_scope uuid NOT NULL,
    idempotency_key text NOT NULL,
    approval_id uuid NOT NULL,
    approval_revision bigint NOT NULL,
    project_id uuid NOT NULL,
    ...
    namespace text NOT NULL,
    release_name text NOT NULL,
    descriptor_yaml bytea NOT NULL,
    values_yaml bytea NOT NULL,
    ...
    lease_until timestamp with time zone,
    worker_contract text,
    worker_renderer_image text,
    worker_renderer_version text,
    worker_policy_version text,
    worker_limits_digest text,
    created_at timestamp with time zone NOT NULL,
    updated_at timestamp with time zone NOT NULL,
    completed_at timestamp with time zone,
    operator_config_digest text NOT NULL,
    worker_operator_config_digest text,
    CONSTRAINT helm_render_commands_attempts_check CHECK (((attempts >= 0) AND (attempts <= 10))),
    CONSTRAINT helm_render_commands_check CHECK (((updated_at >= created_at) AND (available_at >= created_at))),
    CONSTRAINT helm_render_commands_check1 CHECK (((last_failure_code = ''::text) = (consecutive_failures = 0))),
    CONSTRAINT helm_render_commands_check2 CHECK ((((lease_owner IS NULL) AND (lease_until IS NULL) AND (worker_contract IS NULL) AND (worker_renderer_image IS NULL) AND (worker_renderer_version IS NULL) AND (worker_policy_version IS NULL) AND (worker_limits_digest IS NULL)) OR ((lease_owner IS NOT NULL) AND (lease_until IS NOT NULL) AND (worker_contract = 'external-helm-renderer.v1'::text) AND (worker_renderer_image = 'docker.io/alpine/helm:4.2.3'::text) AND (worker_renderer_version = '4.2.3'::text) AND (worker_policy_version = 'external-helm-p0.v1'::text) AND (worker_limits_digest IS NOT NULL) AND (lease_epoch > 0) AND (lease_until > updated_at)))),
    CONSTRAINT helm_render_commands_check3 CHECK ((((state = 'queued'::text) AND (lease_owner IS NULL) AND (completed_at IS NULL)) OR ((state = 'processing'::text) AND (lease_owner IS NOT NULL) AND (completed_at IS NULL) AND (attempts > 0)) OR ((state = ANY (ARRAY['succeeded'::text, 'failed'::text])) AND (lease_owner IS NULL) AND (completed_at IS NOT NULL) AND (completed_at >= created_at)))),
    CONSTRAINT helm_render_commands_consecutive_failures_check CHECK (((consecutive_failures >= 0) AND (consecutive_failures <= 10))),
    CONSTRAINT helm_render_commands_descriptor_digest_check CHECK ((descriptor_digest ~ '^sha256:[0-9a-f]{64}$'::text)),
    CONSTRAINT helm_render_commands_descriptor_yaml_check CHECK (((octet_length(descriptor_yaml) >= 1) AND (octet_length(descriptor_yaml) <= 32768))),
    CONSTRAINT helm_render_commands_idempotency_key_check CHECK (((length(idempotency_key) >= 16) AND (length(idempotency_key) <= 128) AND (idempotency_key ~ '^[A-Za-z0-9][A-Za-z0-9._:-]{15,127}$'::text))),
    CONSTRAINT helm_render_commands_input_digest_check CHECK ((input_digest ~ '^sha256:[0-9a-f]{64}$'::text)),
    CONSTRAINT helm_render_commands_last_failure_code_check CHECK (((last_failure_code = ''::text) OR (last_failure_code ~ '^[a-z][a-z0-9.-]{0,62}$'::text))),
    CONSTRAINT helm_render_commands_lease_epoch_check CHECK ((lease_epoch >= 0)),
    CONSTRAINT helm_render_commands_lease_owner_check CHECK (((lease_owner IS NULL) OR ((length(lease_owner) >= 16) AND (length(lease_owner) <= 128) AND (lease_owner ~ '^[A-Za-z0-9][A-Za-z0-9._:-]{15,127}$'::text)))),
    CONSTRAINT helm_render_commands_namespace_check CHECK (((length(namespace) >= 1) AND (length(namespace) <= 63) AND (namespace ~ '^[a-z0-9](?:[-a-z0-9]{0,61}[a-z0-9])?$'::text))),
    CONSTRAINT helm_render_commands_operator_config_digest_check CHECK ((operator_config_digest ~ '^sha256:[0-9a-f]{64}$'::text)),
    CONSTRAINT helm_render_commands_operator_lease_check CHECK ((((lease_owner IS NULL) AND (worker_operator_config_digest IS NULL)) OR ((lease_owner IS NOT NULL) AND (worker_operator_config_digest = operator_config_digest)))),
    CONSTRAINT helm_render_commands_release_name_check CHECK (((length(release_name) >= 1) AND (length(release_name) <= 63) AND (release_name ~ '^[a-z0-9](?:[-a-z0-9]{0,61}[a-z0-9])?$'::text))),
    CONSTRAINT helm_render_commands_state_check CHECK ((state = ANY (ARRAY['queued'::text, 'processing'::text, 'succeeded'::text, 'failed'::text]))),
    CONSTRAINT helm_render_commands_values_digest_check CHECK ((values_digest ~ '^sha256:[0-9a-f]{64}$'::text)),
    CONSTRAINT helm_render_commands_values_yaml_check CHECK (((octet_length(values_yaml) >= 1) AND (octet_length(values_yaml) <= 262144))),
    CONSTRAINT helm_render_commands_worker_contract_check CHECK (((worker_contract IS NULL) OR (worker_contract = 'external-helm-renderer.v1'::text))),
    CONSTRAINT helm_render_commands_worker_limits_digest_check CHECK (((worker_limits_digest IS NULL) OR (worker_limits_digest ~ '^sha256:[0-9a-f]{64}$'::text))),
    CONSTRAINT helm_render_commands_worker_operator_config_digest_check CHECK (((worker_operator_config_digest IS NULL) OR (worker_operator_config_digest ~ '^sha256:[0-9a-f]{64}$'::text))),
    CONSTRAINT helm_render_commands_worker_policy_version_check CHECK (((worker_policy_version IS NULL) OR (worker_policy_version = 'external-helm-p0.v1'::text))),
    CONSTRAINT helm_render_commands_worker_renderer_image_check CHECK (((worker_renderer_image IS NULL) OR (worker_renderer_image = 'docker.io/alpine/helm:4.2.3'::text))),
    CONSTRAINT helm_render_commands_worker_renderer_version_check CHECK (((worker_renderer_version IS NULL) OR (worker_renderer_version = '4.2.3'::text)))
);


--
-- Name: helm_render_results; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.helm_render_results (
    command_id uuid NOT NULL,
    input_digest text NOT NULL,
    manifest_digest text NOT NULL,
    inventory_digest text NOT NULL,
    rendered_manifests bytea NOT NULL,
    ...
    operator_config_digest text NOT NULL,
    CONSTRAINT helm_render_results_check CHECK (((output_bytes >= 1) AND (output_bytes <= 2097152) AND (output_bytes = octet_length(rendered_manifests)))),
    CONSTRAINT helm_render_results_input_digest_check CHECK ((input_digest ~ '^sha256:[0-9a-f]{64}$'::text)),
    CONSTRAINT helm_render_results_inventory_digest_check CHECK ((inventory_digest ~ '^sha256:[0-9a-f]{64}$'::text)),
    CONSTRAINT helm_render_results_limits_digest_check CHECK ((limits_digest ~ '^sha256:[0-9a-f]{64}$'::text)),
    CONSTRAINT helm_render_results_manifest_digest_check CHECK ((manifest_digest ~ '^sha256:[0-9a-f]{64}$'::text)),
    CONSTRAINT helm_render_results_operator_config_digest_check CHECK ((operator_config_digest ~ '^sha256:[0-9a-f]{64}$'::text)),
    CONSTRAINT helm_render_results_policy_version_check CHECK ((policy_version = 'external-helm-p0.v1'::text)),
    CONSTRAINT helm_render_results_rendered_manifests_check CHECK (((octet_length(rendered_manifests) >= 1) AND (octet_length(rendered_manifests) <= 2097152))),
    CONSTRAINT helm_render_results_renderer_image_check CHECK ((renderer_image = 'docker.io/alpine/helm:4.2.3'::text)),
    CONSTRAINT helm_render_results_renderer_version_check CHECK ((renderer_version = '4.2.3'::text)),
    CONSTRAINT helm_render_results_resource_count_check CHECK (((resource_count >= 1) AND (resource_count <= 128)))
);
...
CREATE TABLE public.runtime_readiness (
    ...
    CONSTRAINT runtime_readiness_runtime_kind_check CHECK ((runtime_kind = ANY (ARRAY['source-build'::text, 'managed-registry'::text, 'git-projection'::text, 'runtime-secret'::text, 'argo-desired-state'::text, 'runtime-registry-pull'::text, 'edge'::text, 'helm-renderer'::text, 'helm-protected-publisher'::text, 'environment-foundation'::text, 'auto-deploy'::text, 'certificate-issuer-observer'::text, 'tls-certificate-observer'::text]))),
...
ALTER TABLE ONLY public.helm_chart_approval_documents
    ADD CONSTRAINT helm_chart_approval_documents_pkey PRIMARY KEY (approval_id, approval_revision);


--
-- Name: helm_chart_approvals helm_chart_approvals_created_by_idempotency_key_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_chart_approvals
    ADD CONSTRAINT helm_chart_approvals_created_by_idempotency_key_key UNIQUE (created_by, idempotency_key);


--
-- Name: helm_chart_approvals helm_chart_approvals_oci_repository_chart_version_manifest__key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_chart_approvals
    ADD CONSTRAINT helm_chart_approvals_oci_repository_chart_version_manifest__key UNIQUE (oci_repository, chart_version, manifest_digest, package_digest, values_schema_digest, renderer_image, policy_version);


--
-- Name: helm_chart_approvals helm_chart_approvals_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_chart_approvals
    ADD CONSTRAINT helm_chart_approvals_pkey PRIMARY KEY (approval_id, revision);
...
--
-- Name: helm_protected_payload_intents helm_protected_payload_intent_environment_id_application_id_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_protected_payload_intents
    ADD CONSTRAINT helm_protected_payload_intent_environment_id_application_id_key UNIQUE (environment_id, application_id, release_generation);


--
-- Name: helm_protected_payload_intents helm_protected_payload_intents_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_protected_payload_intents
    ADD CONSTRAINT helm_protected_payload_intents_pkey PRIMARY KEY (id);


--
-- Name: helm_protected_payload_intents helm_protected_payload_intents_release_revision_id_key; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: helm_release_heads helm_release_heads_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_release_heads
    ADD CONSTRAINT helm_release_heads_pkey PRIMARY KEY (environment_id, application_id);


--
-- Name: helm_release_revisions helm_release_revisions_actor_id_idempotency_key_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_release_revisions
    ADD CONSTRAINT helm_release_revisions_actor_id_idempotency_key_key UNIQUE (actor_id, idempotency_key);


--
-- Name: helm_release_revisions helm_release_revisions_environment_id_application_id_genera_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_release_revisions
    ADD CONSTRAINT helm_release_revisions_environment_id_application_id_genera_key UNIQUE (environment_id, application_id, generation);


--
-- Name: helm_release_revisions helm_release_revisions_id_project_id_environment_id_applica_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_release_revisions
    ADD CONSTRAINT helm_release_revisions_id_project_id_environment_id_applica_key UNIQUE (id, project_id, environment_id, application_id, generation);


--
-- Name: helm_release_revisions helm_release_revisions_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_release_revisions
    ADD CONSTRAINT helm_release_revisions_pkey PRIMARY KEY (id);


--
-- Name: helm_release_revisions helm_release_revisions_render_command_id_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_release_revisions
    ADD CONSTRAINT helm_release_revisions_render_command_id_key UNIQUE (render_command_id);


--
-- Name: helm_render_commands helm_render_commands_idempotency_scope_idempotency_key_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_render_commands
    ADD CONSTRAINT helm_render_commands_idempotency_scope_idempotency_key_key UNIQUE (idempotency_scope, idempotency_key);


--
-- Name: helm_render_commands helm_render_commands_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_render_commands
    ADD CONSTRAINT helm_render_commands_pkey PRIMARY KEY (id);


--
-- Name: helm_render_results helm_render_results_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_render_results
    ADD CONSTRAINT helm_render_results_pkey PRIMARY KEY (command_id);
...
CREATE INDEX helm_protected_application_intents_binding_idx ON public.helm_protected_application_intents USING btree (platform_binding_id, state, created_at);


--
-- Name: helm_protected_application_intents_due_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX helm_protected_application_intents_due_idx ON public.helm_protected_application_intents USING btree (next_attempt_at, created_at, id) WHERE (state = ANY (ARRAY['pending'::text, 'claimed'::text, 'git-committed'::text]));
...
CREATE INDEX helm_protected_payload_intents_binding_idx ON public.helm_protected_payload_intents USING btree (platform_binding_id, state, created_at);


--
-- Name: helm_protected_payload_intents_due_idx; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: helm_release_revisions_history_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX helm_release_revisions_history_idx ON public.helm_release_revisions USING btree (environment_id, application_id, generation DESC);


--
-- Name: helm_render_commands_application_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX helm_render_commands_application_idx ON public.helm_render_commands USING btree (environment_id, application_id, created_at DESC);


--
-- Name: helm_render_commands_approval_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX helm_render_commands_approval_idx ON public.helm_render_commands USING btree (approval_id, approval_revision, created_at DESC);


--
-- Name: helm_render_commands_due_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX helm_render_commands_due_idx ON public.helm_render_commands USING btree (available_at, created_at, id) WHERE (state = ANY (ARRAY['queued'::text, 'processing'::text]));
...
CREATE TRIGGER helm_chart_approval_documents_immutable BEFORE DELETE OR UPDATE ON public.helm_chart_approval_documents FOR EACH ROW EXECUTE FUNCTION public.protect_helm_chart_approval_documents();


--
-- Name: helm_chart_approval_documents helm_chart_approval_documents_validate_insert; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER helm_chart_approval_documents_validate_insert BEFORE INSERT ON public.helm_chart_approval_documents FOR EACH ROW EXECUTE FUNCTION public.validate_helm_chart_approval_document_insert();


--
-- Name: helm_chart_approvals helm_chart_approvals_immutable; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER helm_chart_approvals_immutable BEFORE UPDATE ON public.helm_chart_approvals FOR EACH ROW EXECUTE FUNCTION public.protect_helm_chart_approval();
...
CREATE TRIGGER helm_protected_application_intents_validate BEFORE INSERT OR DELETE OR UPDATE ON public.helm_protected_application_intents FOR EACH ROW EXECUTE FUNCTION public.validate_helm_protected_application_intent();
...
--
-- Name: helm_release_heads helm_release_heads_validate; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER helm_release_heads_validate BEFORE INSERT OR DELETE OR UPDATE ON public.helm_release_heads FOR EACH ROW EXECUTE FUNCTION public.validate_helm_release_head();


--
-- Name: helm_release_revisions helm_release_revisions_validate; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER helm_release_revisions_validate BEFORE INSERT OR DELETE OR UPDATE ON public.helm_release_revisions FOR EACH ROW EXECUTE FUNCTION public.validate_helm_release_revision();


--
-- Name: helm_render_commands helm_render_commands_validate; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER helm_render_commands_validate BEFORE INSERT OR UPDATE ON public.helm_render_commands FOR EACH ROW EXECUTE FUNCTION public.validate_helm_render_command();


--
-- Name: helm_render_results helm_render_results_validate; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER helm_render_results_validate BEFORE INSERT OR UPDATE ON public.helm_render_results FOR EACH ROW EXECUTE FUNCTION public.validate_helm_render_result();
...
--
-- Name: environment_foundation_intents environment_foundation_intent_platform_binding_id_cluster__fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
ALTER TABLE ONLY public.helm_chart_approval_documents
    ADD CONSTRAINT helm_chart_approval_documents_approval_id_approval_revisio_fkey FOREIGN KEY (approval_id, approval_revision) REFERENCES public.helm_chart_approvals(approval_id, revision) ON DELETE RESTRICT;


--
-- Name: helm_chart_approvals helm_chart_approvals_created_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: helm_protected_application_intents helm_protected_application_in_environment_binding_id_envir_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_protected_application_intents
    ADD CONSTRAINT helm_protected_application_in_environment_binding_id_envir_fkey FOREIGN KEY (environment_binding_id, environment_target_ref) REFERENCES public.git_repository_bindings(id, target_ref) ON DELETE RESTRICT;


--
-- Name: helm_protected_application_intents helm_protected_application_in_environment_binding_id_proje_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_protected_application_intents
    ADD CONSTRAINT helm_protected_application_in_environment_binding_id_proje_fkey FOREIGN KEY (environment_binding_id, project_id, environment_id) REFERENCES public.git_repository_bindings(id, project_id, environment_id) ON DELETE RESTRICT;


--
-- Name: helm_protected_application_intents helm_protected_application_in_platform_binding_id_cluster__fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: helm_protected_application_intents helm_protected_application_in_platform_binding_id_platform_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_protected_application_intents
    ADD CONSTRAINT helm_protected_application_in_platform_binding_id_platform_fkey FOREIGN KEY (platform_binding_id, platform_target_ref) REFERENCES public.git_repository_bindings(id, target_ref) ON DELETE RESTRICT;


--
-- Name: helm_protected_application_intents helm_protected_application_in_release_revision_id_project__fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_protected_application_intents
    ADD CONSTRAINT helm_protected_application_in_release_revision_id_project__fkey FOREIGN KEY (release_revision_id, project_id, environment_id, application_id, release_generation) REFERENCES public.helm_release_revisions(id, project_id, environment_id, application_id, generation) ON DELETE RESTRICT;


--
-- Name: helm_protected_application_intents helm_protected_application_intents_payload_intent_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_protected_application_intents
    ADD CONSTRAINT helm_protected_application_intents_payload_intent_id_fkey FOREIGN KEY (payload_intent_id) REFERENCES public.helm_protected_payload_intents(id) ON DELETE RESTRICT;
...
ALTER TABLE ONLY public.helm_protected_payload_intents
    ADD CONSTRAINT helm_protected_payload_intent_environment_binding_id_envir_fkey FOREIGN KEY (environment_binding_id, environment_target_ref) REFERENCES public.git_repository_bindings(id, target_ref) ON DELETE RESTRICT;


--
-- Name: helm_protected_payload_intents helm_protected_payload_intent_environment_binding_id_proje_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_protected_payload_intents
    ADD CONSTRAINT helm_protected_payload_intent_environment_binding_id_proje_fkey FOREIGN KEY (environment_binding_id, project_id, environment_id) REFERENCES public.git_repository_bindings(id, project_id, environment_id) ON DELETE RESTRICT;


--
-- Name: helm_protected_payload_intents helm_protected_payload_intent_platform_binding_id_cluster__fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: helm_protected_payload_intents helm_protected_payload_intent_platform_binding_id_platform_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_protected_payload_intents
    ADD CONSTRAINT helm_protected_payload_intent_platform_binding_id_platform_fkey FOREIGN KEY (platform_binding_id, platform_target_ref) REFERENCES public.git_repository_bindings(id, target_ref) ON DELETE RESTRICT;


--
-- Name: helm_protected_payload_intents helm_protected_payload_intent_release_revision_id_project__fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: helm_release_heads helm_release_heads_application_id_project_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_release_heads
    ADD CONSTRAINT helm_release_heads_application_id_project_id_fkey FOREIGN KEY (application_id, project_id) REFERENCES public.applications(id, project_id) ON DELETE RESTRICT;


--
-- Name: helm_release_heads helm_release_heads_environment_id_project_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_release_heads
    ADD CONSTRAINT helm_release_heads_environment_id_project_id_fkey FOREIGN KEY (environment_id, project_id) REFERENCES public.environments(id, project_id) ON DELETE RESTRICT;


--
-- Name: helm_release_heads helm_release_heads_revision_id_project_id_environment_id_a_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_release_heads
    ADD CONSTRAINT helm_release_heads_revision_id_project_id_environment_id_a_fkey FOREIGN KEY (revision_id, project_id, environment_id, application_id, generation) REFERENCES public.helm_release_revisions(id, project_id, environment_id, application_id, generation) ON DELETE RESTRICT;


--
-- Name: helm_release_revisions helm_release_revisions_actor_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_release_revisions
    ADD CONSTRAINT helm_release_revisions_actor_id_fkey FOREIGN KEY (actor_id) REFERENCES public.users(id) ON DELETE RESTRICT;


--
-- Name: helm_release_revisions helm_release_revisions_application_id_project_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_release_revisions
    ADD CONSTRAINT helm_release_revisions_application_id_project_id_fkey FOREIGN KEY (application_id, project_id) REFERENCES public.applications(id, project_id) ON DELETE RESTRICT;


--
-- Name: helm_release_revisions helm_release_revisions_approval_id_approval_revision_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_release_revisions
    ADD CONSTRAINT helm_release_revisions_approval_id_approval_revision_fkey FOREIGN KEY (approval_id, approval_revision) REFERENCES public.helm_chart_approvals(approval_id, revision) ON DELETE RESTRICT;


--
-- Name: helm_release_revisions helm_release_revisions_base_intent_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_release_revisions
    ADD CONSTRAINT helm_release_revisions_base_intent_fk FOREIGN KEY (base_intent_id) REFERENCES public.helm_protected_application_intents(id) ON DELETE RESTRICT;


--
-- Name: helm_release_revisions helm_release_revisions_environment_id_project_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_release_revisions
    ADD CONSTRAINT helm_release_revisions_environment_id_project_id_fkey FOREIGN KEY (environment_id, project_id) REFERENCES public.environments(id, project_id) ON DELETE RESTRICT;


--
-- Name: helm_release_revisions helm_release_revisions_parent_revision_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_release_revisions
    ADD CONSTRAINT helm_release_revisions_parent_revision_id_fkey FOREIGN KEY (parent_revision_id) REFERENCES public.helm_release_revisions(id) ON DELETE RESTRICT;


--
-- Name: helm_release_revisions helm_release_revisions_project_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_release_revisions
    ADD CONSTRAINT helm_release_revisions_project_id_fkey FOREIGN KEY (project_id) REFERENCES public.projects(id) ON DELETE RESTRICT;


--
-- Name: helm_release_revisions helm_release_revisions_render_command_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_release_revisions
    ADD CONSTRAINT helm_release_revisions_render_command_id_fkey FOREIGN KEY (render_command_id) REFERENCES public.helm_render_commands(id) ON DELETE RESTRICT;


--
-- Name: helm_release_revisions helm_release_revisions_rollback_source_revision_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_release_revisions
    ADD CONSTRAINT helm_release_revisions_rollback_source_revision_id_fkey FOREIGN KEY (rollback_source_revision_id) REFERENCES public.helm_release_revisions(id) ON DELETE RESTRICT;


--
-- Name: helm_render_commands helm_render_commands_application_id_project_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_render_commands
    ADD CONSTRAINT helm_render_commands_application_id_project_id_fkey FOREIGN KEY (application_id, project_id) REFERENCES public.applications(id, project_id) ON DELETE RESTRICT;


--
-- Name: helm_render_commands helm_render_commands_approval_id_approval_revision_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_render_commands
    ADD CONSTRAINT helm_render_commands_approval_id_approval_revision_fkey FOREIGN KEY (approval_id, approval_revision) REFERENCES public.helm_chart_approvals(approval_id, revision) ON DELETE RESTRICT;


--
-- Name: helm_render_commands helm_render_commands_environment_id_project_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_render_commands
    ADD CONSTRAINT helm_render_commands_environment_id_project_id_fkey FOREIGN KEY (environment_id, project_id) REFERENCES public.environments(id, project_id) ON DELETE RESTRICT;


--
-- Name: helm_render_commands helm_render_commands_idempotency_scope_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_render_commands
    ADD CONSTRAINT helm_render_commands_idempotency_scope_fkey FOREIGN KEY (idempotency_scope) REFERENCES public.users(id) ON DELETE RESTRICT;


--
-- Name: helm_render_commands helm_render_commands_project_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_render_commands
    ADD CONSTRAINT helm_render_commands_project_id_fkey FOREIGN KEY (project_id) REFERENCES public.projects(id) ON DELETE RESTRICT;


--
-- Name: helm_render_results helm_render_results_command_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.helm_render_results
    ADD CONSTRAINT helm_render_results_command_id_fkey FOREIGN KEY (command_id) REFERENCES public.helm_render_commands(id) ON DELETE RESTRICT;
