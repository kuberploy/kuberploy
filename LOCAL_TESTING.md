...
OrbStack Docker engine is useful for fast local Buildx work and registry-cache
testing, but OrbStack Kubernetes is not the project integration cluster.
...
| Local container build/cache | OrbStack Docker engine | Dockerfiles, native development images, Buildx behavior, and registry cache import/export |
...
present in the OrbStack engine is not evidence that a Kubernetes node can pull
it from a registry.

## OrbStack local Docker and build-cache lane

Use OrbStack only as a local Docker-compatible engine. Commands should select
...
## Legacy OrbStack Kubernetes helpers

The scripts under `scripts/orbstack/` are retained legacy helpers for the
earlier local Kubernetes walking slice. They deliberately require the exact
`orbstack` Kubernetes context and depend on OrbStack-only routing and image
