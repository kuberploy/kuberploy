...
describe("platform releases page", () => {
  it("shows an operator Helm command and performs no upgrade-history request", async () => {
    ...
    expect(
      screen.getByText(
        'helm upgrade "$RELEASE_NAME" oci://ghcr.io/kuberploy/charts/kuberploy-installer --version 0.1.0-rc.280 --namespace "$NAMESPACE" --values "$VALUES_FILE" --reset-values --server-side=false --wait --timeout 20m',
      ),
    ).toBeVisible();
...
function releaseFixture(): LatestPlatformRelease {
  ...
  const chart = {
    ...
    version: "0.1.0-rc.280",
    ociReference: "ghcr.io/kuberploy/charts/kuberploy-installer:0.1.0-rc.280",
    ociDigest: digest,
    package: "kuberploy-installer-0.1.0-rc.280.tgz",
    packageSha256: digest,
  };
  return {
    currentVersion: "0.1.0-rc.280",
    ...
    release: {
      ...
      notesUrl:
        "https://github.com/kuberploy/kuberploy/releases/tag/v0.1.0-rc.280",
      ...
      manifest: {
        ...
        versions: {
          kuberploy: "0.1.0-rc.280",
          api: "0.1.0-rc.280",
          worker: "0.1.0-rc.280",
          web: "0.1.0-rc.280",
          migration: "0.1.0-rc.280",
          upgrader: "0.1.0-rc.280",
          builderAgent: "0.1.0-rc.280",
          chart: "0.1.0-rc.280",
        },
        compatibility: {
