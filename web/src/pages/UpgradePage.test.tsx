...
describe("platform releases page", () => {
  it("shows an operator Helm command and performs no upgrade-history request", async () => {
    ...
    expect(
      screen.getByText(
        'helm upgrade "$RELEASE_NAME" oci://ghcr.io/kuberploy/charts/kuberploy-installer --version 0.1.0-rc.284 --namespace "$NAMESPACE" --values "$VALUES_FILE" --reset-values --server-side=false --wait --timeout 20m',
      ),
    ).toBeVisible();
...
function releaseFixture(): LatestPlatformRelease {
  ...
  const chart = {
    ...
    version: "0.1.0-rc.284",
    ociReference: "ghcr.io/kuberploy/charts/kuberploy-installer:0.1.0-rc.284",
    ociDigest: digest,
    package: "kuberploy-installer-0.1.0-rc.284.tgz",
    packageSha256: digest,
  };
  return {
    currentVersion: "0.1.0-rc.284",
    ...
    release: {
      ...
      notesUrl:
        "https://github.com/kuberploy/kuberploy/releases/tag/v0.1.0-rc.284",
      ...
      manifest: {
        ...
        versions: {
          kuberploy: "0.1.0-rc.284",
          api: "0.1.0-rc.284",
          worker: "0.1.0-rc.284",
          web: "0.1.0-rc.284",
          migration: "0.1.0-rc.284",
          upgrader: "0.1.0-rc.284",
          builderAgent: "0.1.0-rc.284",
          chart: "0.1.0-rc.284",
        },
        compatibility: {
