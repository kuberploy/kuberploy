...
describe("platform releases page", () => {
  it("shows an operator Helm command and performs no upgrade-history request", async () => {
    ...
    expect(
      screen.getByText(
        'helm upgrade "$RELEASE_NAME" oci://ghcr.io/kuberploy/charts/kuberploy-installer --version 0.1.0-rc.278 --namespace "$NAMESPACE" --values "$VALUES_FILE" --reset-values --server-side=false --wait --timeout 20m',
      ),
    ).toBeVisible();
...
function releaseFixture(): LatestPlatformRelease {
  ...
  const chart = {
    ...
    version: "0.1.0-rc.278",
    ociReference: "ghcr.io/kuberploy/charts/kuberploy-installer:0.1.0-rc.278",
    ociDigest: digest,
    package: "kuberploy-installer-0.1.0-rc.278.tgz",
    packageSha256: digest,
  };
  return {
    currentVersion: "0.1.0-rc.278",
    ...
    release: {
      ...
      notesUrl:
        "https://github.com/kuberploy/kuberploy/releases/tag/v0.1.0-rc.278",
      ...
      manifest: {
        ...
        versions: {
          kuberploy: "0.1.0-rc.278",
          api: "0.1.0-rc.278",
          worker: "0.1.0-rc.278",
          web: "0.1.0-rc.278",
          migration: "0.1.0-rc.278",
          upgrader: "0.1.0-rc.278",
          builderAgent: "0.1.0-rc.278",
          chart: "0.1.0-rc.278",
        },
        compatibility: {
