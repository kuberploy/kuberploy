...
describe("platform releases page", () => {
  it("shows an operator Helm command and performs no upgrade-history request", async () => {
    ...
    expect(
      screen.getByText(
        'helm upgrade "$RELEASE_NAME" oci://ghcr.io/kuberploy/charts/kuberploy-installer --version 0.1.0-rc.402 --namespace "$NAMESPACE" --values "$VALUES_FILE" --reset-values --server-side=false --wait --timeout 65m',
      ),
    ).toBeVisible();
...
function releaseFixture(): LatestPlatformRelease {
  ...
  const chart = {
    ...
    version: "0.1.0-rc.402",
    ociReference: "ghcr.io/kuberploy/charts/kuberploy-installer:0.1.0-rc.402",
    ociDigest: digest,
    package: "kuberploy-installer-0.1.0-rc.402.tgz",
    packageSha256: digest,
  };
  return {
    currentVersion: "0.1.0-rc.402",
    ...
    release: {
      ...
      notesUrl:
        "https://github.com/kuberploy/kuberploy/releases/tag/v0.1.0-rc.402",
      ...
      manifest: {
        ...
        versions: {
          kuberploy: "0.1.0-rc.402",
          api: "0.1.0-rc.402",
          worker: "0.1.0-rc.402",
          web: "0.1.0-rc.402",
          migration: "0.1.0-rc.402",
          builderAgent: "0.1.0-rc.402",
          chart: "0.1.0-rc.402",
        },
        compatibility: {
