...
describe("builder platform settings", () => {
  it("saves concurrency, isolation, and per-container resources from one dedicated page", async () => {
    ...
    const update = vi
      .spyOn(api, "updateBuilderPlatformSettings")
      .mockResolvedValue({
        ...settings,
        revision: 3,
        nodeIsolation: true,
    ...
    await user.click(
      screen.getByRole("checkbox", { name: /Enable node selector/ }),
    );
    await user.click(
      screen.getByRole("button", { name: "Save builder settings" }),
    );
    ...
    expect(update.mock.calls[0]?.[1].dindResources).toEqual(
