        ...
        {sources.map((candidate) => {
          ...
          return (
            <button
              ...
              disabled={!available}
              className="grid grid-cols-[40px_minmax(0,_1fr)_18px] items-start gap-4 min-h-[132px] p-5 border border-line rounded-[10px] text-ink text-left bg-surface cursor-pointer transition-[border-color,background] duration-(--motion-fast) ease-(--ease-standard) hover:border-line-strong hover:bg-surface-soft aria-checked:border-mint aria-checked:bg-mint-soft aria-checked:shadow-[inset_0_0_0_1px_var(--mint)] disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:border-line disabled:hover:bg-surface [&>span:nth-child(2)]:grid [&>span:nth-child(2)]:gap-1.5 [&_strong]:text-sm [&_small]:text-ink-soft [&_small]:text-xs [&_small]:not-italic [&_small]:leading-[1.45] [&_em]:text-xs [&_em]:not-italic [&_em]:leading-[1.45] [&>svg]:w-[18px] [&>svg]:self-center [&>svg]:text-ink-faint"
