...
func (c *PostgreSQLRuntimeBindingCatalog) ArgoRepositoryBindings(
	...
	if c == nil || c.pool == nil || appID <= 0 || !uuidRE.MatchString(platformBindingID) ||
		now.IsZero() || maximumAge <= 0 || maximumAge > time.Hour {
		return nil, ErrInvalid
	}
	rows, err := c.pool.Query(ctx, `SELECT
		b.id::text,b.kind,b.scope_id::text,COALESCE(b.project_id::text,''),COALESCE(b.environment_id::text,''),
	...
	WHERE b.credential_mode='github-app'
	  AND ((b.kind='platform' AND b.id=$2) OR b.kind='environment')
	ORDER BY CASE WHEN b.kind='platform' THEN 0 ELSE 1 END,b.id
	LIMIT $5`, appID, platformBindingID, now.UTC(), now.UTC().Add(-maximumAge), MaximumArgoRepositoryBindings+1)
	if err != nil {
	...
	for _, authority := range authorities {
		if authority.Binding.Kind == gitprojection.BindingPlatform {
			if authority.Binding.ID != platformBindingID {
