...
type transientLeaseLossMaterializer struct {
	calls     atomic.Int64
	recovered chan struct{}
}
...
func (s *transientLeaseLossMaterializer) MaterializeDesiredStateOnce(context.Context, time.Time) (bool, error) {
	if s.calls.Add(1) == 1 {
...
func TestProductionDesiredStateRuntimeReacquiresAfterLeaseLoss(t *testing.T) {
	now := time.Now().UTC().Truncate(time.Microsecond)
	runtime, _ := productionRuntimeFixture(t, now)
	materializer := &transientLeaseLossMaterializer{recovered: make(chan struct{}, 1)}
	runtime.Materializer = materializer
	ctx, cancel := context.WithCancel(t.Context())
	done := make(chan error, 1)
	go func() { done <- runtime.Run(ctx) }()
	...
	case <-time.After(2 * time.Second):
		cancel()
		t.Fatal("runtime terminated instead of reacquiring after a lost lease")
	}
	cancel()
	if err := <-done; !errors.Is(err, context.Canceled) {
		t.Fatalf("recovered runtime did not stop with its context: %v", err)
