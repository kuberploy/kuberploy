...
func TestPostgreSQLDesiredStateFencingSaturationAndExactReadiness(t *testing.T) {
	...
	cleanup := func(cleanupContext context.Context) {
		...
		_, _ = pool.Exec(cleanupContext, `ALTER TABLE argo_desired_state_materialization_receipts DISABLE TRIGGER USER`)
		_, _ = pool.Exec(cleanupContext, `DELETE FROM argo_desired_state_materialization_receipts WHERE platform_binding_id IN ($1,$2) OR environment_binding_id IN ($1,$2)`, pgEnvironmentBindingID, pgPlatformBindingID)
		_, _ = pool.Exec(cleanupContext, `ALTER TABLE argo_desired_state_materialization_receipts ENABLE TRIGGER USER`)
