...
func getDeployment(ctx context.Context, q interface {
	...
	err := q.QueryRow(ctx, `SELECT id,environment_id,application_id,image,replicas,port,environment,route,runtime,state,operation_id,generation,desired_revision,observed_revision,config_raw,config_version,created_at,updated_at FROM deployments WHERE id=$1`, id).Scan(&d.ID, &d.EnvironmentID, &d.ApplicationID, &d.Image, &d.Replicas, &d.Port, &envJSON, &routeJSON, &runtimeJSON, &d.State, &d.OperationID, &d.Generation, &d.DesiredRevision, &d.ObservedRevision, &d.ConfigRaw, &d.ConfigVersion, &d.CreatedAt, &d.UpdatedAt)
	if err == nil {
