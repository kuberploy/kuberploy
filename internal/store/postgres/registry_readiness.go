...
func (s *Store) AcquireManagedRegistryReadiness(ctx context.Context, observation registry.RuntimeWorkerObservation, duration time.Duration) (registry.RuntimeReadinessLease, error) {
	...
	err := s.pool.QueryRow(ctx, `INSERT INTO managed_registry_runtime_readiness(
		registry_target_id,worker_id,worker_epoch,contract_version,config_digest,started_at,observed_at,lease_until
	) VALUES($1,$2,1,$3,$4,$5,$6,$7)
	ON CONFLICT(registry_target_id,worker_id) DO UPDATE SET
		worker_epoch=managed_registry_runtime_readiness.worker_epoch+1,
...
func (s *Store) HeartbeatManagedRegistryReadiness(ctx context.Context, lease registry.RuntimeReadinessLease, observedAt time.Time, duration time.Duration) (registry.RuntimeReadinessLease, error) {
	...
	command, err := s.pool.Exec(ctx, `UPDATE managed_registry_runtime_readiness SET observed_at=$6,lease_until=$7
		WHERE registry_target_id=$1 AND worker_id=$2 AND worker_epoch=$3
...
func (s *Store) ManagedRegistryRuntimeReady(ctx context.Context, identity registry.RuntimeIdentity, now time.Time, maximumAge time.Duration) error {
	...
	err := s.pool.QueryRow(ctx, `SELECT EXISTS(
		SELECT 1 FROM managed_registry_runtime_readiness r
		JOIN registry_targets t ON t.id=r.registry_target_id AND t.mode='managed'
		WHERE r.registry_target_id=$1 AND r.contract_version=$2 AND r.config_digest=$3
