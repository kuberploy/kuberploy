...
func TestTeamAccessSQLPaths(t *testing.T) {
	...
	application, err := st.CreateApplication(ctx, developer.ID, "application", "application", domain.CreateApplication{ProjectID: project.Value.ID, EnvironmentID: environment.Value.ID, Name: "API", Slug: "api", SourceKind: domain.ApplicationSourceGitHub})
	if err != nil {
		t.Fatal(err)
	}
	...
	drafts, err := st.ListDeploymentsForActor(ctx, developer.ID)
	if err != nil || len(drafts) != 1 || drafts[0].ApplicationID != application.Value.ID || drafts[0].State != "stopped" || len(drafts[0].ConfigRaw) != 0 {
		t.Fatalf("source application draft=%+v err=%v", drafts, err)
	}
	...
	exactRelease, exactReleaseOperation, err := st.CreateDeployment(ctx, developer.ID, "exact-config-release", "exact-config-release", "request",
		domain.CreateDeployment{EnvironmentID: environment.Value.ID, ApplicationID: application.Value.ID, Image: updatedImage,
			Runtime: candidate.Runtime, ConfigRaw: append([]byte(nil), candidate.Raw...)}, nil)
	if err != nil || exactRelease.Replay {
		t.Fatalf("exact config release result=%#v operation=%#v err=%v", exactRelease, exactReleaseOperation, err)
	}
	exactReleaseSnapshot, err := st.GetDeploymentForOperation(ctx, exactReleaseOperation.ID)
	if err != nil || string(exactReleaseSnapshot.ConfigRaw) != string(candidate.Raw) || exactReleaseSnapshot.Image != updatedImage {
		t.Fatalf("exact config release regenerated bytes: raw=%q image=%q err=%v", exactReleaseSnapshot.ConfigRaw, exactReleaseSnapshot.Image, err)
	}
