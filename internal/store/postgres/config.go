...
func (s *Store) CreateDeploymentConfigPreview(ctx context.Context, actor string, in domain.CreateConfigPreview, projection *gitprojection.WritePlan, references ...*base.AppConfigReferencePlan) error {
	...
	var currentETag, projectID, environmentID, applicationID, deploymentState string
	err = tx.QueryRow(ctx, `SELECT d.config_etag,e.project_id,d.environment_id::text,d.application_id::text,d.state
		FROM deployments d JOIN environments e ON e.id=d.environment_id
		WHERE d.id=$1 FOR SHARE OF d`, in.DeploymentID).Scan(&currentETag, &projectID, &environmentID, &applicationID, &deploymentState)
	if err != nil {
		return classify(err)
	}
...
func (s *Store) SaveDeploymentConfig(ctx context.Context, actor, key, fingerprint, requestID string, in domain.SaveDeploymentConfig, projection *gitprojection.WritePlan, references ...*base.AppConfigReferencePlan) (base.Result[domain.Deployment], domain.Operation, error) {
	...
	err = tx.QueryRow(ctx, `SELECT d.config_etag,d.config_version,d.generation,e.project_id,d.environment_id::text,d.application_id::text,d.state
		FROM deployments d JOIN environments e ON e.id=d.environment_id
	...
	if referencePlan != nil && usesRuntimeSecrets {
	...
	if _, err = tx.Exec(ctx, `INSERT INTO operations(id,kind,status,target_type,target_id,request_id,generation,progress,created_at,updated_at,finished_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9,$9,$10)`, op.ID, op.Kind, op.Status, op.TargetType, op.TargetID, op.RequestID, op.Generation, progress, now, op.FinishedAt); err != nil {
		return base.Result[domain.Deployment]{}, domain.Operation{}, classify(err)
	}
	d.Runtime = candidateRuntime
	d.Image = candidateImage
	d.Replicas, d.Port, d.Environment = domain.LegacyWorkloadFields(d.Runtime)
	nextState := "pending-git"
	if draftSave {
		nextState = "stopped"
	}
	d.ConfigRaw, d.Generation, d.OperationID, d.State, d.UpdatedAt = append([]byte(nil), in.RawYAML...), generation, op.ID, nextState, now
	...
	if _, err = tx.Exec(ctx, `UPDATE deployments SET image=$2,replicas=$3,port=$4,environment=$5,runtime=$6,state=$7,operation_id=$8,generation=$9,config_raw=$10,config_etag=$11,config_version=$12,updated_at=$13 WHERE id=$1`, d.ID, d.Image, d.Replicas, d.Port, environmentJSON, runtimeJSON, d.State, d.OperationID, generation, d.ConfigRaw, newETag, newVersion, now); err != nil {
		return base.Result[domain.Deployment]{}, domain.Operation{}, classify(err)
	}
	if !draftSave {
	...
	action := "deployment.config.accepted"
	if draftSave {
		action = "deployment.config-draft.saved"
	}
	if err = audit(ctx, tx, actor, action, "deployment", d.ID, requestID, map[string]any{"operationId": op.ID, "baseETag": in.BaseETag, "configVersion": newVersion}); err != nil {
		return base.Result[domain.Deployment]{}, domain.Operation{}, err
	}
