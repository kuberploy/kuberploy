...
func TestFoundationMutationAuthorityIsExactAndCannotBeSubstituted(t *testing.T) {
	...
	binding, err := NewGitHubPlatformBinding(bindingID, repository, "refs/heads/main", now)
	if err != nil {
		t.Fatal(err)
	}
	...
	mutation := Mutation{BindingID: bindingID, OperationID: intentID,
		Path:         PlatformPrefix() + "/argocd/foundations/" + envID + ".yaml",
	...
	tests := map[string]func(*Mutation){
		...
		"path sibling":              func(v *Mutation) { v.Path = PlatformPrefix() + "/argocd/environments/" + envID + ".yaml" },
		"path suffix substitution": func(v *Mutation) {
			v.Path = PlatformPrefix() + "/argocd/foundations/" + envID + "/foundation.yaml"
		},
		"path traversal":   func(v *Mutation) { v.Path = PlatformPrefix() + "/argocd/foundations/../" + envID + ".yaml" },
