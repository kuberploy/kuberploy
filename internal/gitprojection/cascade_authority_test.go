...
func TestHelmCascadeMutationAuthorityIsClosedToFinalizerCAS(t *testing.T) {
	...
	binding, err := NewGitHubPlatformBinding(bindingID, clusterID, repository, "refs/heads/main", time.Date(2026, 8, 15, 0, 0, 0, 0, time.UTC))
	if err != nil {
		t.Fatal(err)
	}
	...
	mutation := Mutation{
		...
		Path:         PlatformPrefix(clusterID) + "/argocd/helm-applications/" + envID + "/" + appID + ".yaml",
	...
	tests := map[string]func(*Mutation){
		...
		"payload path": func(v *Mutation) {
			v.Path = PlatformPrefix(clusterID) + "/helm-manifests/environments/" + envID + "/applications/" + appID + "/revisions/66666666-6666-4666-8666-666666666666/release.yaml"
		},
