...
func (s *PostgresStore) replay(ctx context.Context, tx pgx.Tx, c Command, d string) (MutationResult, bool, error) {
	var old, pid string
	var rev int64
	e := tx.QueryRow(ctx, `SELECT request_digest,profile_id::text,result_revision FROM configuration_profile_commands WHERE actor_id=$1 AND profile_kind='middleware' AND idempotency_key=$2`, c.ActorID, c.IdempotencyKey).Scan(&old, &pid, &rev)
	if errors.Is(e, pgx.ErrNoRows) {
...
func (s *PostgresStore) mutate(ctx context.Context, c Command, action, name string, ref Ref, spec Spec, a []Assignment, clonedFrom *Ref) (MutationResult, error) {
	...
	_, e = tx.Exec(ctx, `INSERT INTO configuration_profile_commands(actor_id,profile_kind,idempotency_key,action,request_digest,profile_id,result_revision,request_id,created_at) VALUES($1,'middleware',$2,$3,$4,$5,$6,$7,$8)`, c.ActorID, c.IdempotencyKey, action, d, pid, resultRev, c.RequestID, c.Now.UTC())
	if e != nil {
		return MutationResult{}, pgerr(e)
	}
...
func (s *PostgresStore) Deactivate(ctx context.Context, c Command, r Ref) (MutationResult, error) {
	...
	_, e = tx.Exec(ctx, `INSERT INTO configuration_profile_commands(actor_id,profile_kind,idempotency_key,action,request_digest,profile_id,result_revision,request_id,created_at) VALUES($1,'middleware',$2,'deactivate',$3,$4,$5,$6,$7)`, c.ActorID, c.IdempotencyKey, d, p.ID, v.Revision, c.RequestID, c.Now.UTC())
	if e != nil {
		return MutationResult{}, pgerr(e)
	}
	_, e = tx.Exec(ctx, `INSERT INTO audit_events(id,actor_id,action,target_type,target_id,request_id,detail,created_at)
