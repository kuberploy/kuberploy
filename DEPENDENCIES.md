...
Status: verified release baseline for 2026-08-06. Prereleases are excluded. Release tags were checked against the upstream project, npm/Go registries were checked for direct packages, chart metadata and package checksums were checked against the publishers' chart indexes, and OCI indexes were checked for the pinned multi-architecture image digests.

This file records the versions selected while the architecture is being designed. It is not a substitute for the immutable, digest-bearing `dependencies.lock.yaml`, package-manager lock files and SBOM that CI will produce for an actual Kuberploy release.

## Selection rule

Kuberploy tracks the newest **mutually compatible stable** release of every direct dependency. For toolchains that publish a production LTS line, the newest stable LTS patch is used. The result is then pinned exactly and resolved to immutable OCI/chart digests. No installer, manifest, build Job or Argo Application may use a floating `latest` tag or resolve "newest" at runtime.
...
| Traefik | [3.7.10](https://github.com/traefik/traefik/releases/tag/v3.7.10) | Official chart [41.1.1](https://github.com/traefik/traefik-helm-chart/releases/tag/v41.1.1) with an explicit, tested 3.7.10 image digest override; see the mismatch note below. |
...
lock. OrbStack supplies a local Docker/Buildx engine for development builds and
...
| Node.js | [24.19.0 LTS](https://nodejs.org/en/blog/release/v24.19.0) | Reproducible production frontend build. Node 26 is the shorter-lived Current line, not the selected production support line. |
...
`@types/node` is intentionally 24.13.3, the newest stable release on the Node 24 type line, rather than the newer Node 26 type line. `@types/react` 19.2.18 and `@types/react-dom` 19.2.4 are the current stable matching React 19 types. The current `web/package.json` direct runtime and development dependencies otherwise match the stable registry tags recorded above.
