...
| Docker Buildx        | [0.21.3](https://github.com/docker/buildx/releases/tag/v0.21.3) | Installed as an exact CPU-only CLI plugin in the builder image. Later release lines unconditionally probe the daemon with a GPU device request when starting the `docker-container` driver.                                           |
...
The official Docker 29.7.1 image bundles Buildx 0.36.0 and the engine embeds BuildKit 0.32.0. Kuberploy deliberately supplies Buildx 0.21.3 to avoid the later unconditional GPU capability probe, while launching BuildKit 0.32.2 as the separately pinned builder. Rootless DinD still requires a privileged Kubernetes container in the documented Docker/ARC pattern; rootless UID mapping does not turn that Pod into an unprivileged workload.
...
| Prisma CLI                   |                                                                                                                                                                                                                                                                                    [7.9.1](https://www.npmjs.com/package/prisma/v/7.9.1) | Migration-only engine in the one-shot Helm Job. It applies reviewed native PostgreSQL SQL; Prisma Client is not used.                                    |
