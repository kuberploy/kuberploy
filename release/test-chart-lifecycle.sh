...
bash -n "${kp_root}/scripts/orbstack/install-platform.sh"
grep -q -- '--rollback-on-failure' "${kp_root}/scripts/orbstack/install-platform.sh"
if grep -q -- '--atomic' "${kp_root}/scripts/orbstack/install-platform.sh"; then
  printf 'OrbStack installer still uses Helm 3 --atomic\n' >&2
  exit 1
fi
