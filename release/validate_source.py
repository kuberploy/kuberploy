...
def main() -> None:
    ...
    install_script = (args.root / "scripts/orbstack/install-platform.sh").read_text(encoding="utf-8")
    if "--atomic" in install_script or "--rollback-on-failure" not in install_script:
        raise SystemExit("OrbStack platform upgrade flags are incompatible with Helm 4")
    print(version)
